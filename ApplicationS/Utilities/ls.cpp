...
int CCmdLS::Invoke(int argc, char* argv[])
{
...
void CCmdLS::ListDirectory(const std::string& path)
{
...
void CCmdLS::PrintFileList(const std::vector<FileEntry>& files)
{
    for (size_t i = 0; i < files.size(); ++i)
    {
...
int CCmdLS::GetFileColor(mode_t mode) const
{
    switch (mode & S_IFMT)
    {
...
PString CCmdLS::FormatFilename(const PString& name, mode_t mode) const
{
    PString prefix;
...
PString CCmdLS::FormatFileSize(off_t size, bool useBlocks) const
{
