...
int CmdLSUSB::Invoke(int argc, char* argv[])
{
...
void CmdLSUSB::PrintTopologyNodeTree(const PString& topologyPath, const std::vector<bool>& treeBranches, bool isLastNode)
{
...
void CmdLSUSB::PrintDeviceTree(const PString& devicePath, const std::vector<bool>& treeBranches, bool isLastDevice)
{
...
void CmdLSUSB::PrintHubInfoTree(const PUSBDeviceInfo& deviceInfo, const std::vector<bool>& treeBranches, bool isLastHubInfo)
{
...
void CmdLSUSB::PrintConfigurationInfoTree(const PUSBDeviceInfo& deviceInfo, const std::vector<bool>& treeBranches, bool isLastConfiguration)
{
...
void CmdLSUSB::PrintInterfaceTree(const PString& interfacePath, const std::vector<bool>& treeBranches, bool isLastInterface)
{
...
void CmdLSUSB::PrintEndpointInfoTree(const PUSBDeviceInterface& deviceInterface, const USB_DescEndpoint& endpointDescriptor, const PUSBHostPipeInfo& hostPipeInfo, const std::vector<bool>& treeBranches, bool isLastEndpoint)
{
...
void CmdLSUSB::PrintHostPipeInfoTree(const PUSBDeviceInterface& deviceInterface, uint8_t endpointAddr, const PUSBHostPipeInfo& hostPipeInfo, const std::vector<bool>& treeBranches, bool isLastHostPipe)
{
...
void CmdLSUSB::PrintInterfaceDescriptorBlockTree(const PUSBDeviceInterface& deviceInterface, size_t descriptorSize, const std::vector<bool>& treeBranches, bool isLastDescriptorBlock)
{
    if (descriptorSize == 0)
    {
...
void CmdLSUSB::PrintDescriptorTree(const USB_DescriptorHeader* descriptor, const std::vector<bool>& treeBranches, bool isLastDescriptor)
{
...
void CmdLSUSB::PrintTopologyNodeCompact(const PString& topologyPath, size_t depth)
{
    const PString nodeName = GetBaseName(topologyPath);
    const PString deviceLinkPath = topologyPath + "/device";
    const PString devicePath = ReadOptionalSymlink(deviceLinkPath);
...
void CmdLSUSB::PrintDeviceCompact(const PString& nodeName, const PString& devicePath, size_t depth)
{
    const PString controlPath = devicePath + "/control";
    const int controlHandle = open(controlPath.c_str(), O_RDONLY);
...
void CmdLSUSB::PrintDeviceInfoCompact(const PString& nodeName, const PString& devicePath, const PUSBDeviceInfo& deviceInfo, const PString& manufacturer, const PString& product, const PString& serialNumber, size_t depth)
{
...
void CmdLSUSB::PrintConfigurationInfoCompact(const PUSBDeviceInfo& deviceInfo, size_t depth)
{
    if (deviceInfo.HasConfigurationDescriptor)
    {
        const USB_DescConfiguration& configurationDescriptor = deviceInfo.ConfigurationDescriptor;
...
void CmdLSUSB::PrintInterfaceListCompact(const PString& devicePath, size_t depth)
{
    const std::vector<PString> interfaceNames = GetSortedPrefixedNames(devicePath, "interface", false);
...
void CmdLSUSB::PrintInterfaceCompact(const PString& interfacePath, size_t depth)
{
    const int interfaceHandle = open(interfacePath.c_str(), O_RDONLY);
...
void CmdLSUSB::PrintInterfaceInfoCompact(const PUSBDeviceInterface& deviceInterface, const PUSBDeviceInterfaceInfo& interfaceInfo, size_t depth)
{
...
void CmdLSUSB::PrintEndpointInfoCompact(const PUSBDeviceInterface& deviceInterface, const USB_DescEndpoint& endpointDescriptor, const PUSBHostPipeInfo& hostPipeInfo, size_t depth)
{
...
void CmdLSUSB::PrintInterfaceDescriptorBlockCompact(const PUSBDeviceInterface& deviceInterface, size_t descriptorSize, size_t depth)
{
    if (descriptorSize == 0) {
        return;
    }
...
PString CmdLSUSB::ReadDeviceString(const PUSBDeviceControl& deviceControl, PUSBDeviceStringID stringID) const
{
...
PString CmdLSUSB::ReadHostPipeDebugEntryString(const PUSBDeviceInterface& deviceInterface, uint8_t endpointAddr, size_t entryIndex, bool readValue) const
{
...
std::vector<NameValue> CmdLSUSB::ReadHostPipeDebugEntries(const PUSBDeviceInterface& deviceInterface, uint8_t endpointAddr) const
{
...
std::vector<PString> CmdLSUSB::GetSortedPrefixedNames(const PString& directoryPath, const char* prefix, bool directoriesOnly)
{
...
bool CmdLSUSB::IsDirectoryEntry(int directoryHandle, const PString& directoryPath, const PString& entryName)
{
...
PString CmdLSUSB::ReadOptionalSymlink(const PString& path)
{
...
std::vector<NameValue> CmdLSUSB::MakeDeviceValues(const PString& devicePath, const PUSBDeviceInfo& deviceInfo, const PString& manufacturer, const PString& product, const PString& serialNumber)
{
    const USB_DescDevice& deviceDescriptor = deviceInfo.DeviceDescriptor;
    const uint16_t vendorID = PLittleEndianToHost(deviceDescriptor.idVendor);
    const uint16_t productID = PLittleEndianToHost(deviceDescriptor.idProduct);
...
std::vector<NameValue> CmdLSUSB::MakeInterfaceValues(const PUSBDeviceInterfaceInfo& interfaceInfo)
{
    const USB_DescInterface& interfaceDescriptor = interfaceInfo.InterfaceDescriptor;
...
std::vector<NameValue> CmdLSUSB::MakeHostPipeValues(const PUSBHostPipeInfo& hostPipeInfo)
{
    return
    {
...
std::vector<NameValue> CmdLSUSB::MakeDescriptorValues(const USB_DescriptorHeader* descriptor)
{
...
void CmdLSUSB::PrintTreeValues(const std::vector<bool>& treeBranches, const std::vector<NameValue>& values)
{
...
void CmdLSUSB::PrintTreeProperty(const std::vector<bool>& treeBranches, bool isLastValue, const PString& name, const PString& value)
{
...
PString CmdLSUSB::MakeTreePrefix(const std::vector<bool>& treeBranches)
{
    PString prefix;
...
bool CmdLSUSB::HasPrefixedNumber(const PString& name, const char* prefix)
{
...
size_t CmdLSUSB::ParsePrefixedNumber(const PString& name, const char* prefix)
{
    size_t value = 0;
    const size_t prefixLength = strlen(prefix);
...
bool CmdLSUSB::ComparePrefixedNumbers(const PString& lhs, const PString& rhs, const char* prefix)
{
...
PString CmdLSUSB::GetBaseName(const PString& path)
{
...
PString CmdLSUSB::MakeIndent(size_t depth)
{
...
PString CmdLSUSB::QuoteString(const PString& value)
{
...
PString CmdLSUSB::FormatDeviceName(const PString& manufacturer, const PString& product)
{
    PString deviceName;
...
PString CmdLSUSB::FormatBCD(uint16_t littleEndianBCD)
{
...
const char* CmdLSUSB::FormatBool(bool value)
{
    return value ? "yes" : "no";
}
...
PString CmdLSUSB::FormatDeviceState(const PUSBDeviceInfo& deviceInfo)
{
    PString state;
...
PString CmdLSUSB::FormatConfigurationAttributes(uint8_t attributes)
{
    PString text;
...
PString CmdLSUSB::FormatHostPipeCompact(const PUSBHostPipeInfo& hostPipeInfo)
{
    if (!hostPipeInfo.IsValid) {
        return "pipe=none";
    }
...
void CmdLSUSB::AppendFlag(PString& text, bool enabled, const char* name)
{
    if (enabled)
    {
        if (!text.empty()) {
...
const char* CmdLSUSB::GetSpeedName(USB_Speed speed)
{
    switch (speed)
    {
...
PString CmdLSUSB::FormatClassCode(USB_ClassCode classCode)
{
...
const char* CmdLSUSB::GetClassName(USB_ClassCode classCode)
{
    switch (classCode)
    {
...
const char* CmdLSUSB::GetRequestDirectionName(USB_RequestDirection direction)
{
    switch (direction)
    {
...
const char* CmdLSUSB::GetURBStateName(USB_URBState state)
{
    switch (state)
    {
...
const char* CmdLSUSB::GetTransferTypeName(USB_TransferType transferType)
{
    switch (transferType)
    {
...
const char* CmdLSUSB::GetEndpointDirectionName(const USB_DescEndpoint& endpointDescriptor)
{
...
const char* CmdLSUSB::GetEndpointSyncName(USB_IsoEndpointSyncType syncType)
{
    switch (syncType)
    {
...
const char* CmdLSUSB::GetEndpointUsageName(USB_EndpointUsageType usageType)
{
    switch (usageType)
    {
...
const char* CmdLSUSB::GetDescriptorTypeName(USB_DescriptorType descriptorType)
{
...
PString CmdLSUSB::FormatDescriptorDetails(const USB_DescriptorHeader* descriptor)
{
    switch (descriptor->bDescriptorType)
    {
        case USB_DescriptorType::INTERFACE:
