...
function(pados_find_build_config outConfigPath)
	...
	list(APPEND configCandidates "${CMAKE_CURRENT_LIST_DIR}/../pados_config.cmake")
...
macro(pados_add_compile_option option define)
    if(${option})
	target_compile_definitions(PadOS_Config INTERFACE ${define}=1)
    endif()
endmacro()
...
macro(pados_add_kernel_module opt lib)
	...
	set_property(GLOBAL APPEND PROPERTY PADOS_KERNEL_MODULE_TARGETS ${lib})
        list(APPEND PADOS_KERNEL_MODULE_EXT_TARGETS "PadOS::${lib}")
	add_library(${lib})
	target_link_libraries(${lib} PUBLIC PadOS_Config)
    endif()
endmacro()

macro(pados_add_kernel_library opt lib)
    if(${opt})
        set_property(GLOBAL APPEND PROPERTY PADOS_KERNEL_LIBRARY_TARGETS ${lib})
        list(APPEND PADOS_KERNEL_LIBRARY_EXT_TARGETS "PadOS::${lib}")
#	add_library(${lib})
	target_link_libraries(${lib} PUBLIC PadOS_Config)
    endif()
endmacro()

macro(pados_add_kernel_external_library opt lib)
    if(${opt})
        set_property(GLOBAL APPEND PROPERTY PADOS_KERNEL_LIBRARY_TARGETS ${lib})
        list(APPEND PADOS_KERNEL_LIBRARY_EXT_TARGETS ${lib})
#	add_library(${lib})
    endif()
endmacro()
