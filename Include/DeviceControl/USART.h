...
inline int USARTIOCTL_SetReadTimeout(int device, TimeValMicros timeout)
{
    bigtime_t micros = timeout.AsMicroSeconds();
    return os::FileIO::DeviceControl(device, USARTIOCTL_SET_READ_TIMEOUT, &micros, sizeof(micros), nullptr, 0);
}

inline int USARTIOCTL_GetReadTimeout(int device, TimeValMicros& outTimeout)
{
    bigtime_t micros;
