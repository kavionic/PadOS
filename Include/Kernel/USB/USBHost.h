...
public:
    ...
    bool            SubmitURB(USB_PipeIndex pipeIndex, USB_RequestDirection direction,
        USB_TransferType enpointType, USBH_InitialTransactionPID initialPID, void* buffer, size_t length,
        USB_TransactionCallback&& callback, size_t receiveCapacity = 0);
    bool            SubmitVectorURB(USB_PipeIndex pipeIndex, USB_RequestDirection direction, USB_TransferType endpointType, USBH_InitialTransactionPID initialPID, const USB_TransferSegment* segments, size_t segmentCount, size_t length, USB_TransactionCallback&& callback);
    bool            ControlSendSetup(USB_PipeIndex pipeIndex, USB_ControlRequest* request, USB_TransactionCallback&& callback);
    bool            ControlSendData(USB_PipeIndex pipeIndex, void* buffer, size_t length, USB_TransactionCallback&& callback);
    bool            ControlReceiveData(USB_PipeIndex pipeIndex, void* buffer, size_t length, USB_TransactionCallback&& callback);
    bool            BulkSendData(USB_PipeIndex pipeIndex, void* buffer, size_t length, USB_TransactionCallback&& callback);
    bool            BulkReceiveData(USB_PipeIndex pipeIndex, void* buffer, size_t length,
        USB_TransactionCallback&& callback, size_t receiveCapacity = 0);
    bool            BulkSendVectorData(USB_PipeIndex pipeIndex, const USB_TransferSegment* segments, size_t segmentCount, size_t length, USB_TransactionCallback&& callback);
