...
)
{
    ...
    {
        printf("TMC2209Driver::Setup(): missing control port.\n");
    ...
    printf("TMC2209Driver::Setup(): IO%lu=%02lx\n", chipAddress, curIO);
...
uint32_t TMC2209Driver::ReadRegister_trw(uint8_t registerAddress) const
{
    ...
    printf("TMC2209Driver::ReadRegister(%d, %d) To many retries.\n", m_ChipAddress, registerAddress);
...
void TMC2209Driver::WriteRegister_trw(uint8_t registerAddress, uint32_t data) const
{
    ...
    for (int i = 0; i < 5; ++i)
    {
        try
        {
            ...
            printf("TMC2209Driver::WriteRegister(%d, %d) failed to update register (%d != %d). Retrying...\n", m_ChipAddress, registerAddress, uint8_t(updateCount), m_CurrentUpdateCount);
            m_CurrentUpdateCount = uint8_t(updateCount);
        }
        PERROR_CATCH([](PErrorCode) {});
    }
    printf("TMC2209Driver::WriteRegister(%d, %d) failed to update register. To many retries.\n", m_ChipAddress, registerAddress);
    PERROR_THROW_CODE(PErrorCode::IOError);
}
