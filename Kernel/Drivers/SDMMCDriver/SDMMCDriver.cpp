...
//
// Copyright (c) 2017-2024 Kurt Skauen
//
		...
		if (hasCard != m_CardInserted || m_CardState != SDMMCDriver::CardState::Ready)
        {
            ...
            switch(m_CardState)
            {
                ...
                case CardState::Ready:
                    kernel_log<PLogSeverity::INFO_LOW_VOL>(LogCategorySDMMCDriver, "SD/MMC card ready: {}", m_SectorCount * BLOCK_SIZE);
                    try {
                        DecodePartitions(true);
                    } catch(...) {}
                    break;
                case CardState::Initializing:
                    kernel_log<PLogSeverity::ERROR>(LogCategorySDMMCDriver, "SD/MMC RestartCard() failed.");
                    break;
                case CardState::Unusable:
                    kernel_log<PLogSeverity::ERROR>(LogCategorySDMMCDriver, "SD/MMC card initialization failed.");
                    snooze_ms(500);
                    break;
...
bool SDMMCDriver::InitializeCard()
{
    ...
    kernel_log<PLogSeverity::INFO_LOW_VOL>(LogCategorySDMMCDriver, "Start SD card install.");
    ...
    if (m_CardType & SDMMCCardType::SD)
    {
        ...
        {
            // It is not a SD card
            kernel_log<PLogSeverity::INFO_LOW_VOL>(LogCategorySDMMCDriver, "Start MMC Install.");
...
void SDMMCDriver::DecodePartitions(bool force)
{
    ...
    kernel_log<PLogSeverity::INFO_LOW_VOL>(LogCategorySDMMCDriver, "SDMMCDriver::DecodePartitions(): Decoding partition table.");
    ...
    for (size_t i = 0 ; i < partitions.size() ; ++i)
    {
        ...
        {
            kernel_log<PLogSeverity::INFO_LOW_VOL>(LogCategorySDMMCDriver,
...
bool SDMMCDriver::OperationalConditionMCI_sd(bool v2)
{
    ...
    for(;;)
    {
        ...
        if (!SendCmd(SDMMC_CMD55_APP_CMD, 0)) {
            kernel_log<PLogSeverity::ERROR>(LogCategorySDMMCDriver, "OperationalConditionMCI_sd() CMD55 failed.");
            return false;
        }
        ...
        if (!SendCmd(SD_MCI_ACMD41_SD_SEND_OP_COND, arg)) {
            kernel_log<PLogSeverity::ERROR>(LogCategorySDMMCDriver, "OperationalConditionMCI_sd() ACMD41 failed.");
            return false;
        }
        ...
        if (kget_monotonic_time() > deadline) {
            kernel_log<PLogSeverity::ERROR>(LogCategorySDMMCDriver, "OperationalConditionMCI_sd(): Timeout ({:#08x})", response);
            return false;
        }
    }
    return true;
...
bool SDMMCDriver::OperationalConditionMCI_mmc()
{
    ...
    for(;;)
    {
        ...
        {
            kernel_log<PLogSeverity::ERROR>(LogCategorySDMMCDriver, "OperationalConditionMCI_mmc() CMD1 MCI failed.");
            return false;
        }
        uint32_t response = GetResponse();
        if (response & OCR_POWER_UP_BUSY)
        ...
        if (kget_monotonic_time() > deadline) {
            kernel_log<PLogSeverity::ERROR>(LogCategorySDMMCDriver, "OperationalConditionMCI_mmc(): Timeout (0x{:08x})", response);
            return false;
        }
    }
    return true;
...
bool SDMMCDriver::OperationalCondition_sdio()
{
    ...
    if (!SendCmd(SDIO_CMD5_SEND_OP_COND, 0)) {
        kernel_log<PLogSeverity::ERROR>(LogCategorySDMMCDriver, "SDMMCDriver::OperationalCondition_sdio:1() CMD5 Fail.");
    ...
    for (;;)
    {
        ...
        if (!SendCmd(SDIO_CMD5_SEND_OP_COND, response & SD_MMC_VOLTAGE_SUPPORT)) {
            kernel_log<PLogSeverity::ERROR>(LogCategorySDMMCDriver, "SDMMCDriver::OperationalCondition_sdio:2() CMD5 Fail.");
            return false;
        }
        ...
        if (kget_monotonic_time() > deadline) {
            kernel_log<PLogSeverity::ERROR>(LogCategorySDMMCDriver, "OperationalCondition_sdio(): Timeout (0x{:08x}).", response);
            return false;
        }
    }
...
bool SDMMCDriver::SetBusWidth_sdio()
{
    ...
    m_BusWidth = 4;
    kernel_log<PLogSeverity::INFO_LOW_VOL>(LogCategorySDMMCDriver, "{}-bit bus width enabled.", m_BusWidth);
    return true;
}
...
bool SDMMCDriver::SetHighSpeed_sd()
{
    ...
    if (GetResponse() & CARD_STATUS_SWITCH_ERROR) {
        kernel_log<PLogSeverity::ERROR>(LogCategorySDMMCDriver, "SetHighSpeed_sd() CMD6 CARD_STATUS_SWITCH_ERROR.");
        return false;
    }
    ...
    if (SD_SW_STATUS_FUN_GRP1_BUSY(switch_status)) {
        kernel_log<PLogSeverity::ERROR>(LogCategorySDMMCDriver, "SetHighSpeed_sd() CMD6 SD_SW_STATUS_FUN_GRP1_BUSY.");
        return false;
    }
...
bool SDMMCDriver::SetBusWidth_mmc(int busWidth)
{
    ...
    if (GetResponse() & CARD_STATUS_SWITCH_ERROR) {
        // Not supported, it is not a protocol error
        kernel_log<PLogSeverity::ERROR>(LogCategorySDMMCDriver, "SetBusWidth_mmc() CMD6 CARD_STATUS_SWITCH_ERROR.");
        return false;
    }
    m_BusWidth = busWidth;
    kernel_log<PLogSeverity::INFO_LOW_VOL>(LogCategorySDMMCDriver, "{}-bit bus width enabled.", m_BusWidth);
...
bool SDMMCDriver::SetHighSpeed_mmc()
{
    ...
    if (GetResponse() & CARD_STATUS_SWITCH_ERROR) {
        // Not supported, it is not a protocol error
        kernel_log<PLogSeverity::ERROR>(LogCategorySDMMCDriver, "SetHighSpeed_mmc() CMD6 CARD_STATUS_SWITCH_ERROR.");
        return false;
    }
    m_HighSpeed = true;
    m_Clock = 52000000lu;
...
bool SDMMCDriver::Cmd8_sd(bool* v2)
{
    *v2 = false;
    ...
    if ((response & (SD_CMD8_MASK_PATTERN | SD_CMD8_MASK_VOLTAGE)) != (SD_CMD8_PATTERN | SD_CMD8_HIGH_VOLTAGE)) {
        kernel_log<PLogSeverity::ERROR>(LogCategorySDMMCDriver, "Cmd8_sd() CMD8 resp32 0x{:08x} UNUSABLE CARD", response);
        return false;
    }
    kernel_log<PLogSeverity::INFO_LOW_VOL>(LogCategorySDMMCDriver, "SD card V2.");
    *v2 = true;
    return true;
}
...
bool SDMMCDriver::Cmd13_sdmmc()
{
    ...
    for(;;)
    {
        ...
        if (kget_monotonic_time() > deadline) {
            kernel_log<PLogSeverity::ERROR>(LogCategorySDMMCDriver, "Cmd13_sdmmc() timeout.");
            return false;
        }
    }
}
...
{
    if (data == nullptr) {
        kernel_log<PLogSeverity::ERROR>(LogCategorySDMMCDriver, "Cmd52_sdio() called with nullptr data.");
        return false;
    }
...
{
    if (size == 0 || size > BLOCK_SIZE) {
        kernel_log<PLogSeverity::ERROR>(LogCategorySDMMCDriver, "Cmd53_sdio() invalid size {}.", size);
        return false;
    }
