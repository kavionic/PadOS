...
{
	REGISTER_KERNEL_LOG_CATEGORY(LOGC_FS,       "FATFS",  PLogSeverity::WARNING);
	REGISTER_KERNEL_LOG_CATEGORY(LOGC_FATTABLE, "FATTBL", PLogSeverity::WARNING);
	REGISTER_KERNEL_LOG_CATEGORY(LOGC_DIR,      "FATDIR", PLogSeverity::WARNING);
	REGISTER_KERNEL_LOG_CATEGORY(LOGC_FILE,     "FATFIL", PLogSeverity::WARNING);
}
...
Ptr<KFSVolume> FATFilesystem::Mount(fs_id volumeID, const char* devicePath, uint32_t flags, const char* args, size_t argLength)
{
    ...
    if (result != PErrorCode::Success)
    {
        ...
        {
            kernel_log(FATFilesystem::LOGC_FS, PLogSeverity::ERROR, "FATFilesystem::Mount(): failed getting device geometry.");
            PERROR_THROW_CODE(result);
        }
    }
    if ((geo.bytes_per_sector != 512) && (geo.bytes_per_sector != 1024) && (geo.bytes_per_sector != 2048)) {
        kernel_log(FATFilesystem::LOGC_FS, PLogSeverity::ERROR, "FATFilesystem::Mount(): unsupported device block size ({}).", geo.bytes_per_sector);
        PERROR_THROW_CODE(PErrorCode::IOError);
    }
    if (geo.removable) {
        kernel_log(LOGC_FS, PLogSeverity::INFO_LOW_VOL, "FATFilesystem::Mount(): {} is removable.", devicePath);
        volumeFlags |= uint32_t(FSVolumeFlags::FS_IS_REMOVABLE);
    }
    if (geo.read_only || (flags & MOUNT_READ_ONLY))
    {
        kernel_log(LOGC_FS, PLogSeverity::INFO_LOW_VOL, "FATFilesystem::Mount(): {} is read-only.", devicePath);
    ...
    else
    {
        ...
        if (deviceFile < 0) {
            kernel_log(FATFilesystem::LOGC_FS, PLogSeverity::ERROR, "FATFilesystem::Mount() unable to reopen {} ({}).", devicePath, strerror(get_last_error()));
    ...
    {
        kernel_log(FATFilesystem::LOGC_FS, PLogSeverity::ERROR, "FATFilesystem::Mount(): volume extends past end of partition ({} > {}).", vol->m_TotalSectors, geo.sector_count);
        PERROR_THROW_CODE(PErrorCode::IOError);
    }
    ...
    if (kpread_trw(deviceFile, buffer.data(), buffer.size(), vol->m_BytesPerSector * (vol->m_ReservedSectors + vol->m_ActiveFAT * vol->m_SectorsPerFAT)) != buffer.size()) {
        kernel_log(FATFilesystem::LOGC_FS, PLogSeverity::ERROR, "FATFilesystem::Mount(): error reading FAT.");
        PERROR_THROW_CODE(PErrorCode::IOError);
    }

    if (buffer[0] != vol->m_MediaDescriptor) {
        kernel_log(FATFilesystem::LOGC_FS, PLogSeverity::ERROR, "FATFilesystem::Mount(): media descriptor mismatch ({:x} != {:x}).", buffer[0], vol->m_MediaDescriptor);
        PERROR_THROW_CODE(PErrorCode::IOError);
    }
    ...
    if (vol->m_FATMirrored)
    {
        ...
        for (i = 0; i < vol->m_FATCount; ++i)
        {
            ...
            {
                kernel_log(LOGC_FS, PLogSeverity::INFO_LOW_VOL, "FATFilesystem::Mount(): checking fat #{}", i);
                ...
                if (kpread_trw(deviceFile, buffer2.data(), buffer2.size(), vol->m_BytesPerSector * (vol->m_ReservedSectors + vol->m_SectorsPerFAT * i)) != buffer2.size()) {
                    kernel_log(FATFilesystem::LOGC_FS, PLogSeverity::ERROR, "FATFilesystem::Mount(): error reading FAT {}", i);
                    PERROR_THROW_CODE(PErrorCode::IOError);
                }

                if (buffer2[0] != vol->m_MediaDescriptor) {
                    kernel_log(FATFilesystem::LOGC_FS, PLogSeverity::ERROR, "FATFilesystem::Mount(): media descriptor mismatch in fat # {} ({:x} != {:x})", i, buffer2[0], vol->m_MediaDescriptor);
                    PERROR_THROW_CODE(PErrorCode::IOError);
                }
                ...
                if (memcmp(buffer.data(), buffer2.data(), buffer.size()) != 0) {
                    kernel_log(FATFilesystem::LOGC_FS, PLogSeverity::WARNING, "FATFilesystem::Mount(): fat {} doesn't match active fat ({}).", i, vol->m_ActiveFAT);
                }
            }
        }
    }
    ...
    if (!vol->m_BCache.SetDevice(deviceFile, vol->m_TotalSectors, vol->m_BytesPerSector)) {
        kernel_log(FATFilesystem::LOGC_FS, PLogSeverity::ERROR, "FATFilesystem::Mount(): error initializing block cache ({}).", strerror(get_last_error()));
        PERROR_THROW_CODE(PErrorCode(get_last_error()));
    }
            ...
            if (fsInfo != nullptr)
            {
                if (fsInfo->m_Signature1 == 0x41615252 && fsInfo->m_Signature2 == 0x61417272 && fsInfo->m_Signature3 == 0xaa550000)
                {
                    ...
                    if (freeClusters != vol->m_FreeClusters)
                    {
                        ...
                        kernel_log(LOGC_FS, KLogSeverity::ERROR, "Mismatching free cluster count in fsinfo block. Found {} should be {}.", vol->m_FreeClusters, freeClusters);
                    }
#endif                    
                ...
                else
                {
                    ...
                    kernel_log(LOGC_FS, PLogSeverity::CRITICAL, "FATFilesystem::Mount(): fsinfo block has invalid magic number {:08x}, {:08x}, {:08x}", signature1, signature2, signature3);
                }
            }
            else
            {
                kernel_log(LOGC_FS, PLogSeverity::ERROR, "FATFilesystem::Mount(): error getting fsinfo sector {:x}", vol->m_FSInfoSector);
            }
        }
        if (!isFreeClustersValid)
        {
    ...
    kernel_log(LOGC_FS, PLogSeverity::INFO_LOW_VOL, "mounting {} (id {:x}, device {:x}, media descriptor {:x})", vol->m_DevicePath.c_str(), vol->m_VolumeID, deviceFile, vol->m_MediaDescriptor);
    kernel_log(LOGC_FS, PLogSeverity::INFO_LOW_VOL, "{:x} bytes/sector, {:x} sectors/cluster", vol->m_BytesPerSector, vol->m_SectorsPerCluster);
    kernel_log(LOGC_FS, PLogSeverity::INFO_LOW_VOL, "{:x} reserved sectors, {:x} total sectors", vol->m_ReservedSectors, vol->m_TotalSectors);
    kernel_log(LOGC_FS, PLogSeverity::INFO_LOW_VOL, "{:x} {}-bit fats, {:x} sectors/fat, {:x} root entries", vol->m_FATCount, vol->m_FATBits, vol->m_SectorsPerFAT, vol->m_RootEntriesCount);
    kernel_log(LOGC_FS, PLogSeverity::INFO_LOW_VOL, "root directory starts at sector {:x} (cluster {:x}), data at sector {:x}", vol->m_RootStart, vol->m_RootINode->m_StartCluster, vol->m_FirstDataSector);
    kernel_log(LOGC_FS, PLogSeverity::INFO_LOW_VOL, "{:x} total clusters, {:x} free", vol->m_TotalClusters, vol->m_FreeClusters);
    kernel_log(LOGC_FS, PLogSeverity::INFO_LOW_VOL, "fat mirroring is {}, fs info sector at sector {:x}", (vol->m_FATMirrored) ? "on" : "off", vol->m_FSInfoSector);
    kernel_log(LOGC_FS, PLogSeverity::INFO_LOW_VOL, "last allocated cluster = {:x}", vol->m_LastAllocatedCluster);
    ...
    kernel_log(LOGC_FS, PLogSeverity::INFO_LOW_VOL, "FATFilesystem::Mount(): Root inode ID = {:x}.", vol->m_RootINode->m_INodeID);
    kernel_log(LOGC_FS, PLogSeverity::INFO_LOW_VOL, "FATFilesystem::Mount(): Volume label [{:11.11}] ({}).", vol->m_VolumeLabel, vol->m_VolumeLabelEntry);
...
void FATFilesystem::Unmount(Ptr<KFSVolume> volume)
{
    ...
    kernel_log(LOGC_FS, PLogSeverity::INFO_LOW_VOL, "FATFilesystem::Unmount(): {:x}", vol->m_VolumeID);
...
void FATFilesystem::Sync(Ptr<KFSVolume> _vol)
{
    ...
    kernel_log(LOGC_FS, PLogSeverity::INFO_LOW_VOL, "FATFilesystem::Sync() called on volume {:x}", vol->m_VolumeID);
...
void FATFilesystem::ReadFSStat(Ptr<KFSVolume> _vol, fs_info* fss)
{
    ...
    kernel_log(LOGC_FS, PLogSeverity::INFO_LOW_VOL, "FATFilesystem::ReadFSStat() called.");
...
void FATFilesystem::WriteFSStat(Ptr<KFSVolume> _vol, const fs_info* fss, uint32_t mask)
{
    ...
    kernel_log(LOGC_FS, PLogSeverity::INFO_LOW_VOL, "FATFilesystem::WriteFSStat() called.");
    ...
    if (mask & WFSSTAT_NAME)
    {
        ...
        memset(name, ' ', 11);
        kernel_log(LOGC_FS, PLogSeverity::INFO_LOW_VOL, "FATFilesystem::WriteFSStat(): setting name to '{}'.", fss->fi_volume_name);
        int i;
        int j;
        ...
        kernel_log(LOGC_FS, PLogSeverity::INFO_LOW_VOL, "FATFilesystem::WriteFSStat(): sanitized to [{:11.11}].", name);
        ...
        if (vol->m_VolumeLabelEntry == -1)
        {
            ...
            {
                kernel_log(LOGC_FS, PLogSeverity::ERROR, "FATFilesystem::WriteFSStat(): label mismatch.");
                PERROR_THROW_CODE(PErrorCode::InvalidArg);
            }
            else
            {
...
Ptr<KINode> FATFilesystem::LocateInode(Ptr<KFSVolume> volume, Ptr<KINode> parent, const char* name, int nameLength)
{
    ...
    kernel_log(FATFilesystem::LOGC_DIR, PLogSeverity::INFO_HIGH_VOL, "FATFilesystem::LocateInode(): find {:x}/{}", dir->m_INodeID, file.c_str());

    const Ptr<FATINode> inode = DoLocateINode(vol, dir, file);
    if (inode == nullptr)
    {
        kernel_log(FATFilesystem::LOGC_DIR, PLogSeverity::INFO_HIGH_VOL, "FATFilesystem::LocateInode(): Error finding inode ID for file {} ({}).", file.c_str(), strerror(get_last_error()));
...
void FATFilesystem::ReleaseInode(KINode* inode)
{
    ...
    if (node->IsDeleted())
    {
        ...
        kernel_log(FATFilesystem::LOGC_FILE, PLogSeverity::INFO_LOW_VOL, "FATFilesystem::ReleaseINode({:x})", node->m_INodeID);

        if (vol->HasFlag(FSVolumeFlags::FS_IS_READONLY)) {
            kernel_log(LOGC_FS, PLogSeverity::ERROR, "FATFilesystem::ReleaseInode(): deleted inode on read-only volume.");
            PERROR_THROW_CODE(PErrorCode::ReadOnlyFilesystem);
        }

        // clear the fat chain
        if (node->m_StartCluster != 0 && !vol->IsDataCluster(node->m_StartCluster)) {
            kernel_log(LOGC_FS, PLogSeverity::ERROR, "FATFilesystem::ReleaseInode(): invalid start cluster {}.", node->m_StartCluster);
        }
        /* XXX: the following assertion was tripped */
        if (node->m_StartCluster == 0 && node->m_Size == 0) {
            kernel_log(LOGC_FS, PLogSeverity::ERROR, "FATFilesystem::ReleaseInode(): inode have start cluster {} and no size.", node->m_StartCluster);
        }
    ...
    kernel_log(LOGC_FS, PLogSeverity::INFO_LOW_VOL, "FATFilesystem::ReleaseInode() (inode ID {:x}).", node->m_INodeID);
}
...
Ptr<KFileNode> FATFilesystem::OpenFile(Ptr<KFSVolume> volume, Ptr<KINode> _node, int openFlags)
{
    ...
    kernel_log(LOGC_FILE, PLogSeverity::INFO_LOW_VOL, "FATFilesystem::OpenFile(): inode ID {:x}, openFlags {:x}", node->m_INodeID, openFlags);

    if (openFlags & O_CREAT)
    {
        kernel_log(LOGC_FILE, PLogSeverity::ERROR, "FATFilesystem::OpenFile(): called with O_CREAT.");
        PERROR_THROW_CODE(PErrorCode::InvalidArg);
    }
    ...
    {
        kernel_log(LOGC_FILE, PLogSeverity::INFO_LOW_VOL, "FATFilesystem::OpenFile(): can't open file for reading with O_TRUNC.");
        PERROR_THROW_CODE(PErrorCode::NoPermission);
    }

    if (openFlags & O_TRUNC)
    {
        kernel_log(LOGC_FILE, PLogSeverity::INFO_LOW_VOL, "FATFilesystem::OpenFile() called with O_TRUNC set.");
...
Ptr<KFileNode> FATFilesystem::CreateFile(Ptr<KFSVolume> volume, Ptr<KINode> parent, const char* _name, int nameLength, int openFlags, int perms)
{
    ...
    if (_name == nullptr) {
        kernel_log(LOGC_FILE, PLogSeverity::CRITICAL, "FATFilesystem::CreateFile() called with null name.");
        PERROR_THROW_CODE(PErrorCode::InvalidArg);
    }
    ...
    kernel_log(LOGC_FILE, PLogSeverity::INFO_LOW_VOL, "FATFilesystem::CreateFile() called: {:x}/{} perms={:o} openFlags={:o}", dir->m_INodeID, name.c_str(), perms, openFlags);

    if (vol->HasFlag(FSVolumeFlags::FS_IS_READONLY)) {
        kernel_log(LOGC_FILE, PLogSeverity::ERROR, "FATFilesystem::CreateFile() called on read-only volume.");
        PERROR_THROW_CODE(PErrorCode::ReadOnlyFilesystem);
    }

    if (dir->IsDeleted()) {
        kernel_log(LOGC_FILE, PLogSeverity::ERROR, "FATFilesystem::CreateFile() called in removed directory.");
        PERROR_THROW_CODE(PErrorCode::NoPermission);
    }

    if ((perms & (S_IWUSR | S_IWGRP | S_IWOTH)) == 0) {
        kernel_log(LOGC_FILE, PLogSeverity::ERROR, "FATFilesystem::CreateFile() called with invalid permission bits ({:x}).", perms);
        PERROR_THROW_CODE(PErrorCode::InvalidArg);
    }

    if ((openFlags & O_ACCMODE) == O_RDONLY) {
        kernel_log(LOGC_FILE, PLogSeverity::ERROR, "invalid permissions used in creating file.");
        PERROR_THROW_CODE(PErrorCode::NoPermission);
    }
    ...
    {
        if (openFlags & O_EXCL) {
            kernel_log(LOGC_FILE, PLogSeverity::ERROR, "FATFilesystem::CreateFile() with O_EXCL called on existing file {}.", name.c_str());
            PERROR_THROW_CODE(PErrorCode::Exist);
        }
        if (file->m_DOSAttribs & FAT_SUBDIR) {
            kernel_log(LOGC_FILE, PLogSeverity::ERROR, "FATFilesystem::CreateFile() called on existing directory.");
            PERROR_THROW_CODE(PErrorCode::NoPermission);
        }
        if (openFlags & O_TRUNC) {
...
void FATFilesystem::CloseFile(Ptr<KFSVolume> volume, KFileNode* file)
{
    ...
    kernel_log(LOGC_FILE, PLogSeverity::INFO_LOW_VOL, "FATFilesystem::CloseFile() (inode ID {:x}).", node->m_INodeID);
}
...
Ptr<KINode> FATFilesystem::LoadInode(Ptr<KFSVolume> volume, ino_t inodeID)
{
    ...
    kernel_log(FATFilesystem::LOGC_FS, PLogSeverity::INFO_HIGH_VOL, "FATFilesystem::LoadInode() (inode ID {:x}).", inodeID);
    ...
    if (IS_ARTIFICIAL_INODEID(loc) || IS_INVALID_INODEID(loc)) {
        kernel_log(FATFilesystem::LOGC_FS, PLogSeverity::CRITICAL, "FATFilesystem::LoadInode(): unknown inode ID {:x} (loc {:x}).", inodeID, loc);
        PERROR_THROW_CODE(PErrorCode::IOError);
    }
    ...
    if (parentINodeID == -1)
    {
        kernel_log(FATFilesystem::LOGC_FS, PLogSeverity::CRITICAL, "FATFilesystem::LoadInode(): unknown directory at cluster {:x}.", DIR_OF_INODEID(loc));
        PERROR_THROW_CODE(PErrorCode::IOError);
    }
    ...
    if (iter.GetCurrentEntry() == nullptr) {
        kernel_log(LOGC_FS, PLogSeverity::ERROR, "FATFilesystem::LoadInode(): error initializing directory for inode {:x} (loc {:x}).", inodeID, loc);
        PERROR_THROW_CODE(PErrorCode::IOError);
    }
    ...
    for (;;)
    {
        ...
        {
            kernel_log(LOGC_FS, PLogSeverity::CRITICAL, "FATFilesystem::LoadInode(): error finding inode {:x} (loc {:x}) ({}).", inodeID, loc, strerror(get_last_error()));
            PERROR_THROW_CODE(PErrorCode::IOError);
        }
...
Ptr<KDirectoryNode> FATFilesystem::OpenDirectory(Ptr<KFSVolume> volume, Ptr<KINode> _node)
{
    ...
    kernel_log(FATFilesystem::LOGC_DIR, PLogSeverity::INFO_HIGH_VOL, "FATFilesystem::OpenDirectory (inode ID {:x}).", node->m_INodeID);

    if (!(node->m_DOSAttribs & FAT_SUBDIR))
    {
        kernel_log(FATFilesystem::LOGC_DIR, PLogSeverity::INFO_LOW_VOL, "FATFilesystem::OpenDirectory() ERROR: inode not a directory.");
...
void FATFilesystem::CreateDirectory(Ptr<KFSVolume> volume, Ptr<KINode> parent, const char* _name, int nameLength, int perms)
{
    ...
    if (dir->IsDeleted())
    {
        kernel_log(LOGC_DIR, PLogSeverity::ERROR, "FATFilesystem::CreateDirectory() called in removed directory.");
        PERROR_THROW_CODE(PErrorCode::NoPermission);
    }
    if (nameLength > 255)
    {
    ...
    kernel_log(LOGC_FILE, PLogSeverity::INFO_LOW_VOL, "FATFilesystem::CreateDirectory() called: {:x}/{} (perm {:o})", dir->m_INodeID, name.c_str(), perms);

    if ((dir->m_DOSAttribs & FAT_SUBDIR) == 0)
    {
        kernel_log(LOGC_DIR, PLogSeverity::ERROR, "FATFilesystem::CreateDirectory(): inode ID {:x} is not a directory.", dir->m_INodeID);
        PERROR_THROW_CODE(PErrorCode::NotDirectory);
    }
    ...
    if (vol->HasFlag(FSVolumeFlags::FS_IS_READONLY))
    {
        kernel_log(LOGC_DIR, PLogSeverity::ERROR, "FATFilesystem::CreateDirectory() called on read-only volume.");
        PERROR_THROW_CODE(PErrorCode::ReadOnlyFilesystem);
    }
    ...
    if(vol->HasINodeIDToLocationIDMapping(dummy->m_INodeID))
    {
        kernel_log(LOGC_FS, PLogSeverity::ERROR, "FATFilesystem::CreateDirectory(): already have ID->location mapping for inode {:x}.", dummy->m_INodeID);
        PERROR_THROW_CODE(PErrorCode::IOError);
    }
    if (vol->HasLocationIDToINodeIDMapping(dummy->m_INodeID))
    {
        kernel_log(LOGC_FS, PLogSeverity::ERROR, "FATFilesystem::CreateDirectory(): already have location->ID mapping for inode {:x}.", dummy->m_INodeID);
        PERROR_THROW_CODE(PErrorCode::IOError);
    }
...
void FATFilesystem::Rename(Ptr<KFSVolume> _vol, Ptr<KINode> _odir, const char* pzOldName, int nOldNameLen, Ptr<KINode> _ndir, const char* pzNewName, int nNewNameLen, bool bMustBeDir)
{
    ...
    kernel_log(LOGC_FILE, PLogSeverity::INFO_LOW_VOL, "FATFilesystem::Rename() called: {:x}/{}->{:x}/{}", odir->m_INodeID, oldname.c_str(), ndir->m_INodeID, newname.c_str());

    if (vol->HasFlag(FSVolumeFlags::FS_IS_READONLY)) {
        kernel_log(LOGC_FILE, PLogSeverity::ERROR, "FATFilesystem::Rename(): called on read-only volume.");
        PERROR_THROW_CODE(PErrorCode::ReadOnlyFilesystem);
    }
    ...
    if (file == nullptr) {
        kernel_log(LOGC_FILE, PLogSeverity::CRITICAL, "FATFilesystem::Rename(): can't find file {} in directory {:x}.", oldname.c_str(), odir->m_INodeID);
        PERROR_THROW_CODE(PErrorCode::IOError);
    }
    ...
    if (file2 != nullptr)
    {
        ...
        {
            kernel_log(LOGC_FILE, PLogSeverity::ERROR, "FATFilesystem::Rename(): destination already occupied by a directory.");
            PERROR_THROW_CODE(PErrorCode::NoPermission);
        }
    ...
    if (file->m_DOSAttribs & FAT_SUBDIR)
    {
        ...
        FATDirectoryEntryCombo* buffer = diri.GetCurrentEntry();
        if (buffer == nullptr) {
            kernel_log(LOGC_FILE, PLogSeverity::ERROR, "FATFilesystem::Rename(): Error opening directory.");
            PERROR_THROW_CODE(PErrorCode::IOError);
        }
        if (memcmp(buffer->m_Normal.m_Filename, "..         ", sizeof(buffer->m_Normal.m_Filename))) {
            kernel_log(LOGC_FILE, PLogSeverity::ERROR, "FATFilesystem::Rename(): Invalid directory.");
            PERROR_THROW_CODE(PErrorCode::IOError);
        }
...
void FATFilesystem::Unlink(Ptr<KFSVolume> vol, Ptr<KINode> dir, const char* _name, int nameLength)
{
    ...
    kernel_log(LOGC_FILE, PLogSeverity::INFO_LOW_VOL, "FATFilesystem::Unlink() called.");
...
{
    String name;
    kernel_log(LOGC_FILE, PLogSeverity::INFO_LOW_VOL, "FATFilesystem::RemoveDirectory() called.");
...
size_t FATFilesystem::Read(Ptr<KFileNode> file, void* buf, size_t len, off64_t pos)
{
    ...
    if (node->m_DOSAttribs & FAT_SUBDIR) {
        kernel_log(LOGC_FILE, PLogSeverity::ERROR, "FATFilesystem::Read() called on directory {:x}.", node->m_INodeID);
        PERROR_THROW_CODE(PErrorCode::IsDirectory);
    }

    kernel_log(LOGC_FILE, PLogSeverity::INFO_LOW_VOL, "FATFilesystem::Read() called {} bytes at {} (inode ID {:x}).", len, pos, node->m_INodeID);
    ...
    if ((fileNode->m_FATIteration == node->m_Iteration) && (pos >= fileNode->m_FATChainIndex * vol->m_BytesPerSector * vol->m_SectorsPerCluster))
    {
        ...
        {
            kernel_log(LOGC_FILE, PLogSeverity::ERROR, "FATFilesystem::Read() invalid m_CachedCluster {} on inode {:x}.", fileNode->m_CachedCluster, node->m_INodeID);
            PERROR_THROW_CODE(PErrorCode::InvalidArg);
        }
    ...
    if ((pos % vol->m_BytesPerSector) != 0)
    {
        ...
        if (buffer.m_Buffer == nullptr) {
            kernel_log(LOGC_FILE, PLogSeverity::ERROR, "FATFilesystem::Read(): error reading cluster {}, sector {}.", iter.m_CurrentCluster, iter.m_CurrentSector);
            PERROR_THROW_CODE(PErrorCode::IOError);
        }
...
size_t FATFilesystem::Write(Ptr<KFileNode> file, const void* buf, size_t len, off64_t pos)
{
    ...
    if (node->m_DOSAttribs & FAT_SUBDIR) {
        kernel_log(LOGC_FILE, PLogSeverity::ERROR, "FATFilesystem::Write() called on directory {:x}.", node->m_INodeID);
        PERROR_THROW_CODE(PErrorCode::IsDirectory);
    }

    kernel_log(LOGC_FILE, PLogSeverity::INFO_LOW_VOL, "FATFilesystem::Write() called {} bytes at {} from buffer at {:x} (inode ID {:x}).", len, pos, (intptr_t)buf, node->m_INodeID);

    if ((fileNode->GetOpenFlags() & O_ACCMODE) == O_RDONLY) {
        kernel_log(LOGC_FILE, PLogSeverity::ERROR, "FATFilesystem::Write(): called on file opened as read-only.");
        PERROR_THROW_CODE(PErrorCode::NoPermission);
    }
    ...
    {
        kernel_log(LOGC_FILE, PLogSeverity::ERROR, "FATFilesystem::Write(): write position exceeds fat limits.");
    ...
    if (node->m_Size && (fileNode->m_FATIteration == node->m_Iteration) && (pos >= fileNode->m_FATChainIndex * vol->m_BytesPerSector * vol->m_SectorsPerCluster))
    {
        if (!vol->IsDataCluster(fileNode->m_CachedCluster))
        {
            kernel_log(LOGC_FILE, PLogSeverity::ERROR, "FATFilesystem::Write() invalid m_CachedCluster {} on inode {:x}.", fileNode->m_CachedCluster, node->m_INodeID);
            PERROR_THROW_CODE(PErrorCode::InvalidArg);
        }
    ...
    if (pos + len > node->m_Size)
    {
        ...
        kernel_log(LOGC_FILE, PLogSeverity::INFO_LOW_VOL, "FATFilesystem::Write(): Setting file size to {} ({} clusters).", node->m_Size, clusters);
    }
    ...
    if ((pos % vol->m_BytesPerSector) != 0)
    {
        ...
        if (buffer.m_Buffer == nullptr)
        {
            kernel_log(LOGC_FILE, PLogSeverity::ERROR, "FATFilesystem::Write(): error writing cluster {}, sector {}.", iter.m_CurrentCluster, iter.m_CurrentSector);
            PERROR_THROW_CODE(PErrorCode::IOError);
        }
...
size_t FATFilesystem::ReadDirectory(Ptr<KFSVolume> volume, Ptr<KDirectoryNode> directory, dirent_t* entry, size_t bufSize)
{
    ...
    kernel_log(FATFilesystem::LOGC_DIR, PLogSeverity::INFO_HIGH_VOL, "FATFilesystem::ReadDirectory(): inode ID {:x}, index {}.", dir->m_INodeID, dirNode->m_CurrentIndex);
    ...
    {
        entryFound = true;
        kernel_log(FATFilesystem::LOGC_DIR, PLogSeverity::INFO_LOW_VOL, "FATFilesystem::ReadDirectory(): found file '{}' / {}.", fileName.c_str(), fileName.size());
    ...
    if (entryFound)
    {
        ...
        kernel_log(FATFilesystem::LOGC_DIR, PLogSeverity::INFO_HIGH_VOL, "FATFilesystem::ReadDirectory(): found file '{}'.", entry->d_name);
        return sizeof(dirent_t);
    }
    else
    {
...
void FATFilesystem::RewindDirectory(Ptr<KFSVolume> _vol, Ptr<KDirectoryNode> _dirNode)
{
    ...
    kernel_log(FATFilesystem::LOGC_DIR, PLogSeverity::INFO_HIGH_VOL, "FATFilesystem::RewindDirectory() (inode ID {:x}).", node->m_INodeID);
...
{
    // no links in fat...
    kernel_log(FATFilesystem::LOGC_FS, PLogSeverity::WARNING, "FATFilesystem::ReadLink() called.");
...
void FATFilesystem::CheckAccess(Ptr<KFSVolume> _vol, Ptr<KINode> _node, int mode)
{
    ...
    kernel_log(FATFilesystem::LOGC_FS, PLogSeverity::INFO_HIGH_VOL, "FATFilesystem::CheckAccess(inode ID {:x}, mode {:x})", node->m_INodeID, mode);

    if ((mode & O_ACCMODE) != O_RDONLY)
    {
        if (vol->HasFlag(FSVolumeFlags::FS_IS_READONLY)) {
            kernel_log(LOGC_FILE, PLogSeverity::ERROR, "FATFilesystem::CheckAccess(): can't write on read-only volume.");
            PERROR_THROW_CODE(PErrorCode::ReadOnlyFilesystem);
        } else if (node->m_DOSAttribs & FAT_READ_ONLY) {
            kernel_log(LOGC_FILE, PLogSeverity::ERROR, "FATFilesystem::CheckAccess(): can't open read-only file for writing.");
            PERROR_THROW_CODE(PErrorCode::NoPermission);
        }
    }
}
...
void FATFilesystem::ReadStat(Ptr<KFSVolume> _vol, Ptr<KINode> _node, struct stat* st)
{
    ...
    kernel_log(LOGC_FILE, PLogSeverity::INFO_LOW_VOL, "FATFilesystem::ReadStat(inode ID {:x})", node->m_INodeID);
...
void FATFilesystem::WriteStat(Ptr<KFSVolume> _vol, Ptr<KINode> _node, const struct stat* st, uint32_t mask)
{
    ...
    kernel_log(LOGC_FILE, PLogSeverity::INFO_LOW_VOL, "FATFilesystem::WriteStat(inode ID {:x})", node->m_INodeID);

    if (vol->HasFlag(FSVolumeFlags::FS_IS_READONLY)) {
        kernel_log(LOGC_FILE, PLogSeverity::ERROR, "FATFilesystem::WriteStat(): read-only volume.");
        PERROR_THROW_CODE(PErrorCode::ReadOnlyFilesystem);
    }

    if (mask & WSTAT_MODE)
    {
        kernel_log(LOGC_FILE, PLogSeverity::INFO_HIGH_VOL, "FATFilesystem::WriteStat(): setting file mode to {:o}.", st->st_mode);
    ...
    if (mask & WSTAT_SIZE)
    {
        kernel_log(LOGC_FILE, PLogSeverity::INFO_LOW_VOL, "FATFilesystem::WriteStat(): setting file size to {:x}.", st->st_size);
        if (node->m_DOSAttribs & FAT_SUBDIR)
        {
            kernel_log(LOGC_FILE, PLogSeverity::ERROR, "FATFilesystem::WriteStat(): can't set file size of directory!");
            PERROR_THROW_CODE(PErrorCode::IsDirectory);
        }
		if (size64_t(st->st_size) > FAT_MAX_FILE_SIZE)
		{
            kernel_log(LOGC_FILE, PLogSeverity::ERROR, "FATFilesystem::WriteStat(): desired file size exceeds fat limit.");
            PERROR_THROW_CODE(PErrorCode::FileTooLarge);
		}
        ...
        kernel_log(LOGC_FILE, PLogSeverity::INFO_LOW_VOL, "FATFilesystem::WriteStat(): setting FAT chain length to {} clusters.", clusters);
        vol->GetFATTable()->SetChainLength(node, clusters, true);
    ...
    if (mask & WSTAT_MTIME)
    {
        kernel_log(LOGC_FILE, PLogSeverity::INFO_HIGH_VOL, "FATFilesystem::WriteStat(): setting modification time.");
...
void FATFilesystem::DeviceControl(Ptr<KFileNode> file, int request, const void* inData, size_t inDataLength, void* outData, size_t outDataLength)
{
	...
	case 100000:
	    kernel_log(FATFilesystem::LOGC_DIR, PLogSeverity::INFO_LOW_VOL, "vol info: {} (device {:x}, media descriptor {:x})", vol->m_DevicePath.c_str(), vol->m_DeviceFile, vol->m_MediaDescriptor);
	    kernel_log(FATFilesystem::LOGC_DIR, PLogSeverity::INFO_LOW_VOL, "{:x} bytes/sector, {:x} sectors/cluster", vol->m_BytesPerSector, vol->m_SectorsPerCluster);
	    kernel_log(FATFilesystem::LOGC_DIR, PLogSeverity::INFO_LOW_VOL, "{:x} reserved sectors, {:x} total sectors", vol->m_ReservedSectors, vol->m_TotalSectors);
	    kernel_log(FATFilesystem::LOGC_DIR, PLogSeverity::INFO_LOW_VOL, "{:x} {}-bit fats, {:x} sectors/fat, {:x} root entries", vol->m_FATCount, vol->m_FATBits, vol->m_SectorsPerFAT, vol->m_RootEntriesCount);
	    kernel_log(FATFilesystem::LOGC_DIR, PLogSeverity::INFO_LOW_VOL, "root directory starts at sector {:x} (cluster {:x}), data at sector {:x}", vol->m_RootStart, vol->m_RootINode->m_StartCluster, vol->m_FirstDataSector);
	    kernel_log(FATFilesystem::LOGC_DIR, PLogSeverity::INFO_LOW_VOL, "{:x} total clusters, {:x} free", vol->m_TotalClusters, vol->m_FreeClusters);
	    kernel_log(FATFilesystem::LOGC_DIR, PLogSeverity::INFO_LOW_VOL, "fat mirroring is {}, fs info sector at sector {:x}", (vol->m_FATMirrored) ? "on" : "off", vol->m_FSInfoSector);
	    kernel_log(FATFilesystem::LOGC_DIR, PLogSeverity::INFO_LOW_VOL, "last allocated cluster = {:x}", vol->m_LastAllocatedCluster);
	    kernel_log(FATFilesystem::LOGC_DIR, PLogSeverity::INFO_LOW_VOL, "root inode id = {:x}", vol->m_RootINode->m_INodeID);
	    kernel_log(FATFilesystem::LOGC_DIR, PLogSeverity::INFO_LOW_VOL, "volume label [{:11.11}]", vol->m_VolumeLabel);
	    return;
			
	case 100001 :
        kernel_log(FATFilesystem::LOGC_DIR, PLogSeverity::INFO_LOW_VOL, "inode id {:x}, dir inode = {:x}", node->m_INodeID, node->m_ParentINodeID);
	    kernel_log(FATFilesystem::LOGC_DIR, PLogSeverity::INFO_LOW_VOL, "si = {:x}, ei = {:x}", node->m_DirStartIndex, node->m_DirEndIndex);
	    kernel_log(FATFilesystem::LOGC_DIR, PLogSeverity::INFO_LOW_VOL, "cluster = {:x} ({:x}), mode = {:x}, size = {:x}", node->m_StartCluster, vol->m_FirstDataSector + vol->m_SectorsPerCluster * (node->m_StartCluster - 2), node->m_DOSAttribs, node->m_Size);
	    vol->GetFATTable()->DumpChain(node->m_StartCluster);
	    return;

	case 100004 :
        kernel_log(FATFilesystem::LOGC_DIR, PLogSeverity::INFO_LOW_VOL, "Dumping inode map for {:x}", vol->m_VolumeID);
	    vol->DumpINodeIDMap();
	    return;

	case 100005 :
        kernel_log(FATFilesystem::LOGC_DIR, PLogSeverity::INFO_LOW_VOL, "Dumping directory map for {:x}", vol->m_VolumeID);
	    vol->DumpDirectoryMap();
	    return;

	default :
        kernel_log(LOGC_FS, PLogSeverity::ERROR, "FATFilesystem::DeviceControl(): vol {:x}, inode {:x} code = {}.", vol->m_VolumeID, node->m_INodeID, request);
        PERROR_THROW_CODE(PErrorCode::InvalidArg);
    }
    PERROR_THROW_CODE(PErrorCode::InvalidArg);
}
    ...
    if (m_FSInfoSector != 0xffff && !HasFlag(FSVolumeFlags::FS_IS_READONLY))
    {
        ...
        if (buffer != nullptr)
        {
            ...
            else
            {
                ...
                kernel_log(FATFilesystem::LOGC_FS, PLogSeverity::CRITICAL, "FATVolume::UpdateFSInfo(): fsinfo block has invalid magic number {:08x}, {:08x}, {:08x}", signature1, signature2, signature3);
            }
        }
        else
        {
            kernel_log(FATFilesystem::LOGC_FS, PLogSeverity::ERROR, "FATVolume::UpdateFSInfo(): error getting fsinfo sector {}.", m_FSInfoSector);
        }
    }
}
...
Ptr<FATINode> FATFilesystem::DoLocateINode(Ptr<FATVolume> vol, Ptr<FATINode> dir, const String& fileName)
{
    ...
    kernel_log(FATFilesystem::LOGC_DIR, PLogSeverity::INFO_HIGH_VOL, "FATFilesystem::DoLocateINode(): {} in {:x}.", fileName.c_str(), dir->m_INodeID);
...
bool FATFilesystem::IsDirectoryEmpty(Ptr<FATVolume> volume, Ptr<FATINode> dir)
{
    ...
    if (iter.GetCurrentEntry() == nullptr) {
        kernel_log(LOGC_DIR, PLogSeverity::ERROR, "FATFilesystem::IsDirectoryEmpty(): error opening directory.");
        PERROR_THROW_CODE(PErrorCode::IOError);
    }
    ...
    for (; i < 3; ++i)
    {
        ...
        {
            kernel_log(LOGC_DIR, PLogSeverity::CRITICAL, "FATFilesystem::IsDirectoryEmpty(): malformed directory.");
            PERROR_THROW_CODE(PErrorCode::NotDirectory);
        }
    }
    return false;
...
void FATFilesystem::CreateDirectoryEntry(Ptr<FATVolume> vol, Ptr<FATINode> parent, Ptr<FATINode> node, const String& name, uint32_t* startIndex, uint32_t* endIndex)
{
    ...
    {
        kernel_log(FATFilesystem::LOGC_DIR, PLogSeverity::INFO_HIGH_VOL, "FATFilesystem::CreateDirectoryEntry(): {} already found in directory {:x}.", name.c_str(), parent->m_INodeID);
        PERROR_THROW_CODE(PErrorCode::Exist);
    }
    ...
    {
        kernel_log(FATFilesystem::LOGC_DIR, PLogSeverity::CRITICAL, "FATFilesystem::CreateDirectoryEntry(): Error converting utf8 name '{}' to UNICODE. Result to long.", name.c_str());
        PERROR_THROW_CODE(PErrorCode::NameTooLong);
    }
    longName[len++] = 0;
    ...
    if (FATDirectoryIterator::RequiresLongName(longName.data(), len))
    {
        ...
        for (int i = 1; i <= 10; ++i)
        {
            ...
            kernel_log(FATFilesystem::LOGC_DIR, PLogSeverity::INFO_HIGH_VOL, "FATFilesystem::CreateDirectoryEntry(): trying short name [{:11.11}].", shortName);
    ...
    kernel_log(FATFilesystem::LOGC_DIR, PLogSeverity::INFO_LOW_VOL, "FATFilesystem::CreateDirectoryEntry(): creating directory entry [{:11.11}].", shortName);
...
void FATFilesystem::DoCreateDirectoryEntry(Ptr<FATVolume> vol, Ptr<FATINode> dir, FATNewDirEntryInfo* info, const char shortName[11], const wchar16_t* longName, uint32_t len, uint32_t* startIndex, uint32_t* endIndex)
{
    ...
    {
        kernel_log(LOGC_DIR, PLogSeverity::CRITICAL, "FATFilesystem::DoCreateDirectoryEntry(): for bad cluster ({}).", info->cluster);
        PERROR_THROW_CODE(PErrorCode::IOError);
    }
    ...
    *startIndex = 0;
    bool last_entry = true;
    {
        ...
        while (diri.GetCurrentEntry() != nullptr)
        {
            ...
            if (loops++ > 1000) {
                kernel_log(LOGC_DIR, PLogSeverity::CRITICAL, "FATFilesystem::DoCreateDirectoryEntry(): infinit loop.");
                break;
            }
    ...
    *endIndex = *startIndex + required_entries - 1;
    ...
    kernel_log(FATFilesystem::LOGC_DIR, PLogSeverity::INFO_HIGH_VOL, "FATFilesystem::DoCreateDirectoryEntry(): directory entry runs from {:x} to {:x} (dirsize = {}){}", *startIndex, *endIndex, dir->m_Size, last_entry ? " (last entry)" : "");
    ...
    if (*endIndex * sizeof(FATDirectoryEntry) >= dir->m_Size)
    {
        ...
        if (IS_FIXED_ROOT(dir->m_StartCluster)) {
            kernel_log(FATFilesystem::LOGC_DIR, PLogSeverity::WARNING, "FATFilesystem::DoCreateDirectoryEntry(): out of space in root directory.");
        ...
        kernel_log(FATFilesystem::LOGC_DIR, PLogSeverity::INFO_HIGH_VOL, "FATFilesystem::DoCreateDirectoryEntry(): expanding directory from {} to {} clusters.", dir->m_Size/vol->m_BytesPerSector/vol->m_SectorsPerCluster, clusters_needed);
    ...
    if (buffer == nullptr) { // This should never happen.
        kernel_log(FATFilesystem::LOGC_DIR, PLogSeverity::CRITICAL, "FATFilesystem::DoCreateDirectoryEntry(): Iteration failed.");
        PERROR_THROW_CODE(PErrorCode::IOError);
    }
...
{
    kernel_log(FATFilesystem::LOGC_DIR, PLogSeverity::INFO_HIGH_VOL, "FATFilesystem::CompactDirectory(): compacting directory with inode ID {:x}.", dir->m_INodeID);
    ...
    while (diri.GetCurrentEntry() != nullptr)
    {
        ...
        if ( loops++ > 1000 ) {
            kernel_log(LOGC_DIR, PLogSeverity::CRITICAL, "FATFilesystem::CompactDirectory(): infinit loop." );
            break;
        }
        ...
        else
        {
            ...
            {
                kernel_log(FATFilesystem::LOGC_DIR, PLogSeverity::INFO_HIGH_VOL, "FATFilesystem::CompactDirectory(): shrinking directory to {} clusters.", clusters);
...
void FATFilesystem::EraseDirectoryEntry(Ptr<FATVolume> vol, Ptr<FATINode> node)
{
    ...
    kernel_log(FATFilesystem::LOGC_DIR, PLogSeverity::INFO_HIGH_VOL, "FATFilesystem::EraseDirectoryEntry(): erasing directory entries {} through {}.", node->m_DirStartIndex, node->m_DirEndIndex);
    {
        ...
        if (buffer == nullptr) {
            kernel_log(LOGC_DIR, PLogSeverity::ERROR, "FATFilesystem::EraseDirectoryEntry(): error reading directory.");
            PERROR_THROW_CODE(PErrorCode::IOError);
        }
        if (!diri.GetNextLFNEntry(&info, nullptr)) {
            PERROR_THROW_CODE(PErrorCode::NoEntry);
    ...
    if (info.m_StartIndex != node->m_DirStartIndex || info.m_EndIndex != node->m_DirEndIndex)
    {
        ...
        kernel_log(LOGC_DIR, PLogSeverity::CRITICAL, "erase_dir_entry: directory entry doesn't match.");
        PERROR_THROW_CODE(PErrorCode::IOError);
    }
...
void FATFilesystem::DoUnlink(Ptr<KFSVolume> _vol, Ptr<KINode> _dir, const String& name, bool removeFile)
{
    ...
    kernel_log(LOGC_FILE, PLogSeverity::INFO_LOW_VOL, "FATFilesystem::DoUnlink(): {:x}/{}", dir->m_INodeID, name.c_str());

    if (vol->HasFlag(FSVolumeFlags::FS_IS_READONLY)) {
        kernel_log(LOGC_DIR, PLogSeverity::ERROR, "FATFilesystem::DoUnlink(): read-only volume.");
        PERROR_THROW_CODE(PErrorCode::ReadOnlyFilesystem);
    }
    ...
    Ptr<FATINode> file = DoLocateINode(vol, dir, name);
    if (file == nullptr) {
        kernel_log(LOGC_FILE, PLogSeverity::CRITICAL, "FATFilesystem::DoUnlink(): can't find file {} in directory {:x}.", name.c_str(), dir->m_INodeID);
        PERROR_THROW_CODE(PErrorCode::NoEntry);
    }
    ...
    else
    {
        ...
        if (file->m_INodeID == vol->m_RootINode->m_INodeID) {
            kernel_log(LOGC_FILE, PLogSeverity::ERROR, "FATFilesystem::DoUnlink(): don't call this on the root directory.");
            PERROR_THROW_CODE(PErrorCode::NoPermission);
        }
