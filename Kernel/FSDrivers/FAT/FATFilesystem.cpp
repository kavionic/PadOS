...
{
	REGISTER_KERNEL_LOG_CATEGORY(LOGC_FS,       PLogSeverity::WARNING);
	REGISTER_KERNEL_LOG_CATEGORY(LOGC_FATTABLE, PLogSeverity::WARNING);
	REGISTER_KERNEL_LOG_CATEGORY(LOGC_DIR,      PLogSeverity::WARNING);
	REGISTER_KERNEL_LOG_CATEGORY(LOGC_FILE,     PLogSeverity::WARNING);
}
...
Ptr<KFSVolume> FATFilesystem::Mount(fs_id volumeID, const char* devicePath, uint32_t flags, const char* args, size_t argLength)
{
    ...
    if ((geo.bytes_per_sector != 512) && (geo.bytes_per_sector != 1024) && (geo.bytes_per_sector != 2048)) {
        kernel_log(FATFilesystem::LOGC_FS, PLogSeverity::ERROR, "FATFilesystem::Mount(): unsupported device block size (%lu).\n", geo.bytes_per_sector);
        PERROR_THROW_CODE(PErrorCode::IOError);
    }
    if (geo.removable) {
        kernel_log(LOGC_FS, PLogSeverity::INFO_LOW_VOL, "FATFilesystem::Mount(): %s is removable.\n", devicePath);
        volumeFlags |= uint32_t(FSVolumeFlags::FS_IS_REMOVABLE);
    }
    if (geo.read_only || (flags & MOUNT_READ_ONLY))
    {
        kernel_log(LOGC_FS, PLogSeverity::INFO_LOW_VOL, "FATFilesystem::Mount(): %s is read-only.\n", devicePath);
    ...
    else
    {
        ...
        if (deviceFile < 0) {
            kernel_log(FATFilesystem::LOGC_FS, PLogSeverity::ERROR, "FATFilesystem::Mount() unable to reopen %s (%s)\n", devicePath, strerror(get_last_error()));
    ...
    {
        kernel_log(FATFilesystem::LOGC_FS, PLogSeverity::ERROR, "FATFilesystem::Mount(): volume extends past end of partition (%ld > %Ld)\n", vol->m_TotalSectors, geo.sector_count);
        PERROR_THROW_CODE(PErrorCode::IOError);
    }
    ...
    if (buffer[0] != vol->m_MediaDescriptor) {
        kernel_log(FATFilesystem::LOGC_FS, PLogSeverity::ERROR, "FATFilesystem::Mount(): media descriptor mismatch (%x != %x)\n", buffer[0], vol->m_MediaDescriptor);
        PERROR_THROW_CODE(PErrorCode::IOError);
    }
    ...
    if (vol->m_FATMirrored)
    {
        ...
        for (i = 0; i < vol->m_FATCount; ++i)
        {
            ...
            {
                kernel_log(LOGC_FS, PLogSeverity::INFO_LOW_VOL, "FATFilesystem::Mount(): checking fat #%ld\n", i);
                ...
                if (kpread_trw(deviceFile, buffer2.data(), buffer2.size(), vol->m_BytesPerSector * (vol->m_ReservedSectors + vol->m_SectorsPerFAT * i)) != buffer2.size()) {
                    kernel_log(FATFilesystem::LOGC_FS, PLogSeverity::ERROR, "FATFilesystem::Mount(): error reading FAT %ld\n", i);
                    PERROR_THROW_CODE(PErrorCode::IOError);
                }

                if (buffer2[0] != vol->m_MediaDescriptor) {
                    kernel_log(FATFilesystem::LOGC_FS, PLogSeverity::ERROR, "FATFilesystem::Mount(): media descriptor mismatch in fat # %ld (%x != %x)\n", i, buffer2[0], vol->m_MediaDescriptor);
                    PERROR_THROW_CODE(PErrorCode::IOError);
                }
                ...
                if (memcmp(buffer.data(), buffer2.data(), buffer.size()) != 0) {
                    kernel_log(FATFilesystem::LOGC_FS, PLogSeverity::WARNING, "FATFilesystem::Mount(): fat %d doesn't match active fat (%d)\n", i, vol->m_ActiveFAT);
                }
            }
        }
    }
    ...
    if (!vol->m_BCache.SetDevice(deviceFile, vol->m_TotalSectors, vol->m_BytesPerSector)) {
        kernel_log(FATFilesystem::LOGC_FS, PLogSeverity::ERROR, "FATFilesystem::Mount(): error initializing block cache (%s)\n", strerror(get_last_error()));
        PERROR_THROW_CODE(PErrorCode(get_last_error()));
    }
        ...
        if (vol->m_FSInfoSector != 0xffff)
        {
            ...
            if (fsInfo != nullptr)
            {
                ...
                else
                {
                    ...
                    kernel_log(LOGC_FS, PLogSeverity::CRITICAL, "FATFilesystem::Mount(): fsinfo block has invalid magic number %08x, %08x, %08x\n", signature1, signature2, signature3);
                }
            }
            else
            {
                kernel_log(LOGC_FS, PLogSeverity::ERROR, "FATFilesystem::Mount(): error getting fsinfo sector %x\n", vol->m_FSInfoSector);
            }
        }
        if (!isFreeClustersValid)
        {
    ...
    kernel_log(LOGC_FS, PLogSeverity::INFO_LOW_VOL, "mounting %s (id %x, device %x, media descriptor %x)\n", vol->m_DevicePath.c_str(), vol->m_VolumeID, deviceFile, vol->m_MediaDescriptor);
    kernel_log(LOGC_FS, PLogSeverity::INFO_LOW_VOL, "%lx bytes/sector, %lx sectors/cluster\n", vol->m_BytesPerSector, vol->m_SectorsPerCluster);
    kernel_log(LOGC_FS, PLogSeverity::INFO_LOW_VOL, "%lx reserved sectors, %lx total sectors\n", vol->m_ReservedSectors, vol->m_TotalSectors);
    kernel_log(LOGC_FS, PLogSeverity::INFO_LOW_VOL, "%lx %d-bit fats, %lx sectors/fat, %lx root entries\n", vol->m_FATCount, vol->m_FATBits, vol->m_SectorsPerFAT, vol->m_RootEntriesCount);
    kernel_log(LOGC_FS, PLogSeverity::INFO_LOW_VOL, "root directory starts at sector %lx (cluster %lx), data at sector %lx\n", vol->m_RootStart, vol->m_RootINode->m_StartCluster, vol->m_FirstDataSector);
    kernel_log(LOGC_FS, PLogSeverity::INFO_LOW_VOL, "%lx total clusters, %lx free\n", vol->m_TotalClusters, vol->m_FreeClusters);
    kernel_log(LOGC_FS, PLogSeverity::INFO_LOW_VOL, "fat mirroring is %s, fs info sector at sector %x\n", (vol->m_FATMirrored) ? "on" : "off", vol->m_FSInfoSector);
    kernel_log(LOGC_FS, PLogSeverity::INFO_LOW_VOL, "last allocated cluster = %lx\n", vol->m_LastAllocatedCluster);
    ...
    kernel_log(LOGC_FS, PLogSeverity::INFO_LOW_VOL, "FATFilesystem::Mount(): Root inode ID = %" PRIx64 "\n", vol->m_RootINode->m_INodeID);
    kernel_log(LOGC_FS, PLogSeverity::INFO_LOW_VOL, "FATFilesystem::Mount(): Volume label [%11.11s] (%ld).\n", vol->m_VolumeLabel, vol->m_VolumeLabelEntry);
...
void FATFilesystem::Unmount(Ptr<KFSVolume> volume)
{
    ...
    kernel_log(LOGC_FS, PLogSeverity::INFO_LOW_VOL, "FATFilesystem::Unmount(): %x\n", vol->m_VolumeID);
...
void FATFilesystem::Sync(Ptr<KFSVolume> _vol)
{
    ...
    kernel_log(LOGC_FS, PLogSeverity::INFO_LOW_VOL, "FATFilesystem::Sync() called on volume %x\n", vol->m_VolumeID);
...
void FATFilesystem::WriteFSStat(Ptr<KFSVolume> _vol, const fs_info* fss, uint32_t mask)
{
    ...
    if (mask & WFSSTAT_NAME)
    {
        ...
        memset(name, ' ', 11);
        kernel_log(LOGC_FS, PLogSeverity::INFO_LOW_VOL, "FATFilesystem::WriteFSStat(): setting name to %s\n", fss->fi_volume_name);
        int i;
        int j;
        ...
        kernel_log(LOGC_FS, PLogSeverity::INFO_LOW_VOL, "FATFilesystem::WriteFSStat(): sanitized to [%11.11s].\n", name);
...
Ptr<KINode> FATFilesystem::LocateInode(Ptr<KFSVolume> volume, Ptr<KINode> parent, const char* name, int nameLength)
{
    ...
    kernel_log(FATFilesystem::LOGC_DIR, PLogSeverity::INFO_HIGH_VOL, "FATFilesystem::LocateInode(): find %" PRIx64 "/%s\n", dir->m_INodeID, file.c_str());

    const Ptr<FATINode> inode = DoLocateINode(vol, dir, file);
    if (inode == nullptr)
    {
        kernel_log(FATFilesystem::LOGC_DIR, PLogSeverity::INFO_HIGH_VOL, "FATFilesystem::LocateInode(): Error finding inode ID for file %s (%s)\n", file.c_str(), strerror(get_last_error()));
...
void FATFilesystem::ReleaseInode(KINode* inode)
{
    ...
    if (node->IsDeleted())
    {
        ...
        kernel_log(FATFilesystem::LOGC_FILE, PLogSeverity::INFO_LOW_VOL, "FATFilesystem::ReleaseINode(%" PRIx64 ")\n", node->m_INodeID);
        ...
        if (node->m_StartCluster != 0 && !vol->IsDataCluster(node->m_StartCluster)) {
            kernel_log(LOGC_FS, PLogSeverity::ERROR, "FATFilesystem::ReleaseInode(): invalid start cluster %ld\n", node->m_StartCluster);
        }
        /* XXX: the following assertion was tripped */
        if (node->m_StartCluster == 0 && node->m_Size == 0) {
            kernel_log(LOGC_FS, PLogSeverity::ERROR, "FATFilesystem::ReleaseInode(): inode have start cluster %ld and no size.\n", node->m_StartCluster);
        }
    ...
    kernel_log(LOGC_FS, PLogSeverity::INFO_LOW_VOL, "FATFilesystem::ReleaseInode() (inode ID %" PRIx64 ")\n", node->m_INodeID);
}
...
Ptr<KFileNode> FATFilesystem::OpenFile(Ptr<KFSVolume> volume, Ptr<KINode> _node, int openFlags)
{
    ...
    kernel_log(LOGC_FILE, PLogSeverity::INFO_LOW_VOL, "FATFilesystem::OpenFile(): inode ID %" PRIx64 ", openFlags %x\n", node->m_INodeID, openFlags);
...
Ptr<KFileNode> FATFilesystem::CreateFile(Ptr<KFSVolume> volume, Ptr<KINode> parent, const char* _name, int nameLength, int openFlags, int perms)
{
    ...
    kernel_log(LOGC_FILE, PLogSeverity::INFO_LOW_VOL, "FATFilesystem::CreateFile() called: %" PRIx64 "/%s perms=%o openFlags=%o\n", dir->m_INodeID, name.c_str(), perms, openFlags);
    ...
    if ((perms & (S_IWUSR | S_IWGRP | S_IWOTH)) == 0) {
        kernel_log(LOGC_FILE, PLogSeverity::ERROR, "FATFilesystem::CreateFile() called with invalid permission bits (%x).\n", perms);
        PERROR_THROW_CODE(PErrorCode::InvalidArg);
    }
    ...
    {
        if (openFlags & O_EXCL) {
            kernel_log(LOGC_FILE, PLogSeverity::ERROR, "FATFilesystem::CreateFile() with O_EXCL called on existing file %s.\n", name.c_str());
...
void FATFilesystem::CloseFile(Ptr<KFSVolume> volume, KFileNode* file)
{
    ...
    kernel_log(LOGC_FILE, PLogSeverity::INFO_LOW_VOL, "FATFilesystem::CloseFile() (inode ID %" PRId64 ")\n", node->m_INodeID);
}
...
Ptr<KINode> FATFilesystem::LoadInode(Ptr<KFSVolume> volume, ino_t inodeID)
{
    ...
    kernel_log(FATFilesystem::LOGC_FS, PLogSeverity::INFO_HIGH_VOL, "FATFilesystem::LoadInode() (inode ID %" PRIx64 ").\n", inodeID);
    ...
    if (IS_ARTIFICIAL_INODEID(loc) || IS_INVALID_INODEID(loc)) {
	    kernel_log(FATFilesystem::LOGC_FS, PLogSeverity::CRITICAL, "FATFilesystem::LoadInode(): unknown inode ID %" PRIx64 " (loc %" PRIx64 ").\n", inodeID, loc);
        PERROR_THROW_CODE(PErrorCode::IOError);
    }
    ...
    if (parentINodeID == -1)
    {
	    kernel_log(FATFilesystem::LOGC_FS, PLogSeverity::CRITICAL, "FATFilesystem::LoadInode(): unknown directory at cluster %lx.\n", DIR_OF_INODEID(loc));
        PERROR_THROW_CODE(PErrorCode::IOError);
    }
    ...
    if (iter.GetCurrentEntry() == nullptr) {
	    kernel_log(LOGC_FS, PLogSeverity::ERROR, "FATFilesystem::LoadInode(): error initializing directory for inode %" PRIx64 " (loc %" PRIx64 ").\n", inodeID, loc);
        PERROR_THROW_CODE(PErrorCode::IOError);
    }
    ...
    for (;;)
    {
        ...
        {
            kernel_log(LOGC_FS, PLogSeverity::CRITICAL, "FATFilesystem::LoadInode(): error finding inode %" PRIx64 " (loc %" PRIx64 ") (%s).\n", inodeID, loc, strerror(get_last_error()));
            PERROR_THROW_CODE(PErrorCode::IOError);
        }
...
Ptr<KDirectoryNode> FATFilesystem::OpenDirectory(Ptr<KFSVolume> volume, Ptr<KINode> _node)
{
    ...
    kernel_log(FATFilesystem::LOGC_DIR, PLogSeverity::INFO_HIGH_VOL, "FATFilesystem::OpenDirectory (inode ID %" PRIx64 ").\n", node->m_INodeID);
...
void FATFilesystem::CreateDirectory(Ptr<KFSVolume> volume, Ptr<KINode> parent, const char* _name, int nameLength, int perms)
{
    ...
    kernel_log(LOGC_FILE, PLogSeverity::INFO_LOW_VOL, "FATFilesystem::CreateDirectory() called: %" PRIx64 "/%s (perm %o)\n", dir->m_INodeID, name.c_str(), perms);

    if ((dir->m_DOSAttribs & FAT_SUBDIR) == 0)
    {
        kernel_log(LOGC_DIR, PLogSeverity::ERROR, "FATFilesystem::CreateDirectory(): inode ID %" PRIx64 " is not a directory\n", dir->m_INodeID);
        PERROR_THROW_CODE(PErrorCode::NotDirectory);
    }
    ...
    if(vol->HasINodeIDToLocationIDMapping(dummy->m_INodeID))
    {
        kernel_log(LOGC_FS, PLogSeverity::ERROR, "FATFilesystem::CreateDirectory(): already have ID->location mapping for inode %" PRIx64 "\n", dummy->m_INodeID);
        PERROR_THROW_CODE(PErrorCode::IOError);
    }
    if (vol->HasLocationIDToINodeIDMapping(dummy->m_INodeID))
    {
        kernel_log(LOGC_FS, PLogSeverity::ERROR, "FATFilesystem::CreateDirectory(): already have location->ID mapping for inode %" PRIx64 "\n", dummy->m_INodeID);
        PERROR_THROW_CODE(PErrorCode::IOError);
    }
...
void FATFilesystem::Rename(Ptr<KFSVolume> _vol, Ptr<KINode> _odir, const char* pzOldName, int nOldNameLen, Ptr<KINode> _ndir, const char* pzNewName, int nNewNameLen, bool bMustBeDir)
{
    ...
    kernel_log(LOGC_FILE, PLogSeverity::INFO_LOW_VOL, "FATFilesystem::Rename() called: %" PRIx64 "/%s->%Lx/%s\n", odir->m_INodeID, oldname.c_str(), ndir->m_INodeID, newname.c_str());
    ...
    if (file == nullptr) {
        kernel_log(LOGC_FILE, PLogSeverity::CRITICAL, "FATFilesystem::Rename(): can't find file %s in directory %" PRIx64 "\n", oldname.c_str(), odir->m_INodeID);
        PERROR_THROW_CODE(PErrorCode::IOError);
    }
...
size_t FATFilesystem::Read(Ptr<KFileNode> file, void* buf, size_t len, off64_t pos)
{
    ...
    if (node->m_DOSAttribs & FAT_SUBDIR) {
        kernel_log(LOGC_FILE, PLogSeverity::ERROR, "FATFilesystem::Read() called on directory %" PRIx64 "\n", node->m_INodeID);
        PERROR_THROW_CODE(PErrorCode::IsDirectory);
    }

    kernel_log(LOGC_FILE, PLogSeverity::INFO_LOW_VOL, "FATFilesystem::Read() called %d bytes at %" PRId64 " (inode ID %" PRIx64 ")\n", len, pos, node->m_INodeID);
    ...
    if ((fileNode->m_FATIteration == node->m_Iteration) && (pos >= fileNode->m_FATChainIndex * vol->m_BytesPerSector * vol->m_SectorsPerCluster))
    {
        ...
        {
            kernel_log(LOGC_FILE, PLogSeverity::ERROR, "FATFilesystem::Read() invalid m_CachedCluster %ld on inode %" PRIx64 "\n", fileNode->m_CachedCluster, node->m_INodeID);
            PERROR_THROW_CODE(PErrorCode::InvalidArg);
        }
    ...
    if ((pos % vol->m_BytesPerSector) != 0)
    {
        ...
        if (buffer.m_Buffer == nullptr) {
            kernel_log(LOGC_FILE, PLogSeverity::ERROR, "FATFilesystem::Read(): error reading cluster %lx, sector %lx\n", iter.m_CurrentCluster, iter.m_CurrentSector);
            PERROR_THROW_CODE(PErrorCode::IOError);
        }
...
size_t FATFilesystem::Write(Ptr<KFileNode> file, const void* buf, size_t len, off64_t pos)
{
    ...
    if (node->m_DOSAttribs & FAT_SUBDIR) {
        kernel_log(LOGC_FILE, PLogSeverity::ERROR, "FATFilesystem::Write() called on directory %" PRIx64 "\n", node->m_INodeID);
        PERROR_THROW_CODE(PErrorCode::IsDirectory);
    }

    kernel_log(LOGC_FILE, PLogSeverity::INFO_LOW_VOL, "FATFilesystem::Write() called %d bytes at %" PRId64 " from buffer at %lx (inode ID %" PRIx64 ")\n", len, pos, (uint32_t)buf, node->m_INodeID);
    ...
    if (node->m_Size && (fileNode->m_FATIteration == node->m_Iteration) && (pos >= fileNode->m_FATChainIndex * vol->m_BytesPerSector * vol->m_SectorsPerCluster))
    {
        if (!vol->IsDataCluster(fileNode->m_CachedCluster))
        {
            kernel_log(LOGC_FILE, PLogSeverity::ERROR, "FATFilesystem::Write() invalid m_CachedCluster %ld on inode %" PRIx64 "\n", fileNode->m_CachedCluster, node->m_INodeID);
            PERROR_THROW_CODE(PErrorCode::InvalidArg);
        }
    ...
    if (pos + len > node->m_Size)
    {
        ...
        kernel_log(LOGC_FILE, PLogSeverity::INFO_LOW_VOL, "FATFilesystem::Write(): Setting file size to %ld (%ld clusters)\n", node->m_Size, clusters);
    }
    ...
    if ((pos % vol->m_BytesPerSector) != 0)
    {
        ...
        if (buffer.m_Buffer == nullptr)
        {
            kernel_log(LOGC_FILE, PLogSeverity::ERROR, "FATFilesystem::Write(): error writing cluster %lx, sector %lx\n", iter.m_CurrentCluster, iter.m_CurrentSector);
            PERROR_THROW_CODE(PErrorCode::IOError);
        }
...
size_t FATFilesystem::ReadDirectory(Ptr<KFSVolume> volume, Ptr<KDirectoryNode> directory, dirent_t* entry, size_t bufSize)
{
    ...
    kernel_log(FATFilesystem::LOGC_DIR, PLogSeverity::INFO_HIGH_VOL, "FATFilesystem::ReadDirectory(): inode ID %" PRIx64 ", index %lx\n", dir->m_INodeID, dirNode->m_CurrentIndex);
    ...
    {
        entryFound = true;
        kernel_log(FATFilesystem::LOGC_DIR, PLogSeverity::INFO_LOW_VOL, "FATFilesystem::ReadDirectory(): found file '%s' / %" PRId32 "\n", fileName.c_str(), fileName.size());
    ...
    if (entryFound)
    {
        ...
        kernel_log(FATFilesystem::LOGC_DIR, PLogSeverity::INFO_HIGH_VOL, "FATFilesystem::ReadDirectory(): found file %s\n", entry->d_name);
        return sizeof(dirent_t);
    }
    else
    {
...
void FATFilesystem::RewindDirectory(Ptr<KFSVolume> _vol, Ptr<KDirectoryNode> _dirNode)
{
    ...
    kernel_log(FATFilesystem::LOGC_DIR, PLogSeverity::INFO_HIGH_VOL, "FATFilesystem::RewindDirectory() (inode ID %" PRIx64 ")\n", node->m_INodeID);
...
void FATFilesystem::CheckAccess(Ptr<KFSVolume> _vol, Ptr<KINode> _node, int mode)
{
    ...
    kernel_log(FATFilesystem::LOGC_FS, PLogSeverity::INFO_HIGH_VOL, "FATFilesystem::CheckAccess(inode ID %" PRIx64 ", mode %x)\n", node->m_INodeID, mode);
...
void FATFilesystem::ReadStat(Ptr<KFSVolume> _vol, Ptr<KINode> _node, struct stat* st)
{
    ...
    kernel_log(LOGC_FILE, PLogSeverity::INFO_LOW_VOL, "FATFilesystem::ReadStat(inode ID %" PRIx64 ")\n", node->m_INodeID);
...
void FATFilesystem::WriteStat(Ptr<KFSVolume> _vol, Ptr<KINode> _node, const struct stat* st, uint32_t mask)
{
    ...
    kernel_log(LOGC_FILE, PLogSeverity::INFO_LOW_VOL, "FATFilesystem::WriteStat(inode ID %" PRIx64 ")\n", node->m_INodeID);
    ...
    if (mask & WSTAT_MODE)
    {
        kernel_log(LOGC_FILE, PLogSeverity::INFO_HIGH_VOL, "FATFilesystem::WriteStat(): setting file mode to %o\n", st->st_mode);
    ...
    if (mask & WSTAT_SIZE)
    {
        kernel_log(LOGC_FILE, PLogSeverity::INFO_LOW_VOL, "FATFilesystem::WriteStat(): setting file size to %Lx\n", st->st_size);
        if (node->m_DOSAttribs & FAT_SUBDIR)
        {
        ...
        kernel_log(LOGC_FILE, PLogSeverity::INFO_LOW_VOL, "FATFilesystem::WriteStat(): setting FAT chain length to %lx clusters\n", clusters);
        vol->GetFATTable()->SetChainLength(node, clusters, true);
...
void FATFilesystem::DeviceControl(Ptr<KFileNode> file, int request, const void* inData, size_t inDataLength, void* outData, size_t outDataLength)
{
	...
	default :
	    kernel_log(LOGC_FS, PLogSeverity::ERROR, "FATFilesystem::DeviceControl(): vol %x, inode %" PRIx64 " code = %d\n", vol->m_VolumeID, node->m_INodeID, request);
        PERROR_THROW_CODE(PErrorCode::InvalidArg);
    }
    PERROR_THROW_CODE(PErrorCode::InvalidArg);
}
    ...
    if (m_FSInfoSector != 0xffff && !HasFlag(FSVolumeFlags::FS_IS_READONLY))
    {
        ...
        if (buffer != nullptr)
        {
            ...
            else
            {
                ...
                kernel_log(FATFilesystem::LOGC_FS, PLogSeverity::CRITICAL, "FATVolume::UpdateFSInfo(): fsinfo block has invalid magic number %08x, %08x, %08x\n", signature1, signature2, signature3);
            }
        }
        else
        {
            kernel_log(FATFilesystem::LOGC_FS, PLogSeverity::ERROR, "FATVolume::UpdateFSInfo(): error getting fsinfo sector %x\n", m_FSInfoSector);
        }
    }
}
...
Ptr<FATINode> FATFilesystem::DoLocateINode(Ptr<FATVolume> vol, Ptr<FATINode> dir, const String& fileName)
{
    ...
    kernel_log(FATFilesystem::LOGC_DIR, PLogSeverity::INFO_HIGH_VOL, "FATFilesystem::DoLocateINode(): %s in %" PRIx64 "\n", fileName.c_str(), dir->m_INodeID);
...
void FATFilesystem::CreateDirectoryEntry(Ptr<FATVolume> vol, Ptr<FATINode> parent, Ptr<FATINode> node, const String& name, uint32_t* startIndex, uint32_t* endIndex)
{
    ...
    {
        kernel_log(FATFilesystem::LOGC_DIR, PLogSeverity::INFO_HIGH_VOL, "FATFilesystem::CreateDirectoryEntry(): %s already found in directory %" PRIx64 "\n", name.c_str(), parent->m_INodeID);
        PERROR_THROW_CODE(PErrorCode::Exist);
    }
    ...
    {
        kernel_log(FATFilesystem::LOGC_DIR, PLogSeverity::CRITICAL, "FATFilesystem::CreateDirectoryEntry(): Error converting utf8 name '%s' to UNICODE. Result to long.\n", name.c_str());
        PERROR_THROW_CODE(PErrorCode::NameTooLong);
    }
    longName[len++] = 0;
    ...
    if (FATDirectoryIterator::RequiresLongName(longName.data(), len))
    {
        ...
        for (int i = 1; i <= 10; ++i)
        {
            ...
            kernel_log(FATFilesystem::LOGC_DIR, PLogSeverity::INFO_HIGH_VOL, "FATFilesystem::CreateDirectoryEntry(): trying short name %11.11s\n", shortName);
    ...
    kernel_log(FATFilesystem::LOGC_DIR, PLogSeverity::INFO_LOW_VOL, "FATFilesystem::CreateDirectoryEntry(): creating directory entry (%11.11s)\n", shortName);
...
void FATFilesystem::DoCreateDirectoryEntry(Ptr<FATVolume> vol, Ptr<FATINode> dir, FATNewDirEntryInfo* info, const char shortName[11], const wchar16_t* longName, uint32_t len, uint32_t* startIndex, uint32_t* endIndex)
{
    ...
    {
        kernel_log(LOGC_DIR, PLogSeverity::CRITICAL, "FATFilesystem::DoCreateDirectoryEntry(): for bad cluster (%lx)\n", info->cluster);
        PERROR_THROW_CODE(PErrorCode::IOError);
    }
    ...
    *endIndex = *startIndex + required_entries - 1;
    ...
    kernel_log(FATFilesystem::LOGC_DIR, PLogSeverity::INFO_HIGH_VOL, "FATFilesystem::DoCreateDirectoryEntry(): directory entry runs from %lx to %lx (dirsize = %" PRIx64 ") (is%s last entry)\n", *startIndex, *endIndex, dir->m_Size, last_entry ? "" : "n't");
    ...
    if (*endIndex * sizeof(FATDirectoryEntry) >= dir->m_Size)
    {
        ...
        kernel_log(FATFilesystem::LOGC_DIR, PLogSeverity::INFO_HIGH_VOL, "FATFilesystem::DoCreateDirectoryEntry(): expanding directory from %Lx to %lx clusters\n", dir->m_Size/vol->m_BytesPerSector/vol->m_SectorsPerCluster, clusters_needed);
...
{
    kernel_log(FATFilesystem::LOGC_DIR, PLogSeverity::INFO_HIGH_VOL, "FATFilesystem::CompactDirectory(): compacting directory with inode ID %" PRIx64 "\n", dir->m_INodeID);
    ...
    while (diri.GetCurrentEntry() != nullptr)
    {
        ...
        else
        {
            ...
            {
                kernel_log(FATFilesystem::LOGC_DIR, PLogSeverity::INFO_HIGH_VOL, "FATFilesystem::CompactDirectory(): shrinking directory to %lx clusters\n", clusters);
...
void FATFilesystem::EraseDirectoryEntry(Ptr<FATVolume> vol, Ptr<FATINode> node)
{
    ...
    kernel_log(FATFilesystem::LOGC_DIR, PLogSeverity::INFO_HIGH_VOL, "FATFilesystem::EraseDirectoryEntry(): erasing directory entries %lx through %lx\n", node->m_DirStartIndex, node->m_DirEndIndex);
    {
...
void FATFilesystem::DoUnlink(Ptr<KFSVolume> _vol, Ptr<KINode> _dir, const String& name, bool removeFile)
{
    ...
    kernel_log(LOGC_FILE, PLogSeverity::INFO_LOW_VOL, "FATFilesystem::DoUnlink(): %" PRIx64 "/%s\n", dir->m_INodeID, name.c_str());
    ...
    Ptr<FATINode> file = DoLocateINode(vol, dir, name);
    if (file == nullptr) {
        kernel_log(LOGC_FILE, PLogSeverity::CRITICAL, "FATFilesystem::DoUnlink(): can't find file %s in directory %" PRIx64 "\n", name.c_str(), dir->m_INodeID);
        PERROR_THROW_CODE(PErrorCode::NoEntry);
    }
