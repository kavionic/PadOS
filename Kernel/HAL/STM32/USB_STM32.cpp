...
bool USB_STM32::Setup(USB_OTG_ID portID, USB_Mode mode, const PinMuxTarget& pinDM, const PinMuxTarget& pinDP, const PinMuxTarget& pinID, DigitalPinID pinVBus, bool useSOF)
{
    ...
    {
        DigitalPin(DigitalPinID::A9).SetDirection(DigitalPinDirection_e::Analog);
    }
    else
    {
    ...
    if (mode == USB_Mode::Device) {
        m_Port->GUSBCFG |= USB_OTG_GUSBCFG_FDMOD;
    } else if (mode == USB_Mode::Host) {
    ...
    while ((m_Port->GRSTCTL & USB_OTG_GRSTCTL_AHBIDL) == 0U) {}
    m_Port->GRSTCTL |= USB_OTG_GRSTCTL_CSRST;
    while ((m_Port->GRSTCTL & USB_OTG_GRSTCTL_CSRST) == USB_OTG_GRSTCTL_CSRST) {}

    // Restart PHY clock.
    *m_PCGCCTL = 0;

    // Clear all interrupts.
    m_Port->GINTSTS |= m_Port->GINTSTS;
    ...
    m_Port->GINTMSK |= USB_OTG_GINTMSK_OTGINT | USB_OTG_GINTMSK_MMISM | USB_OTG_GINTMSK_USBRST | USB_OTG_GINTMSK_ENUMDNEM | USB_OTG_GINTMSK_USBSUSPM | USB_OTG_GINTMSK_WUIM | USB_OTG_GINTMSK_RXFLVLM | (useSOF ? USB_OTG_GINTMSK_SOFM : 0);
    ...
    register_irq_handler(irq, &USB_STM32::IRQCallback, this);
...
void USB_STM32::EndpointStall(uint8_t endpointAddr)
{
...
void USB_STM32::EndpointClearStall(uint8_t endpointAddr)
{
...
uint32_t USB_STM32::CalculateRXFIFOSize(uint32_t maxEndpointSize) const
{
...
void USB_STM32::SetSpeed(USB_Speed speed)
{
...
USB_Speed USB_STM32::GetSpeed() const
{
...
void USB_STM32::SetTurnaround(USB_Speed speed)
{
    uint32_t turnaround;
...
void USB_STM32::EndpointDisable(uint8_t endpointAddr, bool stall)
{
    const uint8_t epNum = USB_ADDRESS_EPNUM(endpointAddr);
...
bool USB_STM32::EndpointOpen(const USB_DescEndpoint& endpointDescriptor)
{
...
void USB_STM32::EndpointClose(uint8_t endpointAddr)
{
    const uint8_t epNum = USB_ADDRESS_EPNUM(endpointAddr);
...
void USB_STM32::EndpointCloseAll()
{
...
bool USB_STM32::EndpointTransfer(uint8_t endpointAddr, uint8_t* buffer, size_t totalLength)
{
    const uint8_t epNum = USB_ADDRESS_EPNUM(endpointAddr);
...
bool USB_STM32::SetAddress(uint8_t deviceAddr)
{
...
void USB_STM32::EndpointSchedulePackets(uint8_t endpointAddr, uint32_t packetCount, uint32_t totalLength)
{
    const uint8_t epNum = USB_ADDRESS_EPNUM(endpointAddr);
...
IRQResult USB_STM32::IRQCallback(IRQn_Type irq, void* userData)
{
    return static_cast<USB_STM32*>(userData)->HandleIRQ();
}
...
IRQResult USB_STM32::HandleIRQ()
{
    ...
    if (intStatus & USB_OTG_GINTSTS_ENUMDNE)   // ENUMDNE indicates the end of reset on the USB. Speed has been detected.
    {
        ...
        const USB_Speed speed = GetSpeed();
        SetTurnaround(speed);
        IRQBusReset(speed);
    }
    ...
    if(intStatus & USB_OTG_GINTSTS_DISCINT)
    {
        IRQDeviceDisconnected();
    }
    ...
    {
        IRQIncompleteIsochronousINTransfer();
    }
...
void USB_STM32::HandleRXFIFOLevelIRQ()
{
    ...
    switch (packetStatus)
    {
        ...
        case USB_PKTSTS_OUT_DATA_RCV:
        {
            ...
            ReadFromFIFO(xfer.Buffer, packetLength);
        ...
        case USB_PKTSTS_SETUP_DATA_RCV:
            ReadFromFIFO(&m_ControlRequestPackage, sizeof(m_ControlRequestPackage));
            break;
        default:
            break;
    }
...
void USB_STM32::HandleOutEndpointIRQ()
{
    for (uint8_t epNum = 0; epNum < ENDPOINT_COUNT; ++epNum)
    {
        ...
        if (m_Device->DAINT & (1 << (USB_OTG_DAINT_OEPINT_Pos + epNum)))
        {
            ...
            if (m_OutEndpoints[epNum].DOEPINT & USB_OTG_DOEPINT_STUP)
            {
                ...
                IRQControlRequestReceived(m_ControlRequestPackage);
            }
            ...
            if (m_OutEndpoints[epNum].DOEPINT & USB_OTG_DOEPINT_XFRC)
            {
                ...
                if ((epNum == 0) && m_Enpoint0OutPending != 0) {
                    ...
                    IRQTransferComplete(epNum, xfer.TotalLength, USB_TransferResult::Success);
                }
            }
        }
    }
...
void USB_STM32::HandleInEndpointIRQ()
{
    for (uint8_t epNum = 0; epNum < ENDPOINT_COUNT; ++epNum)
    {
        ...
        if (m_Device->DAINT & (1 << (USB_OTG_DAINT_IEPINT_Pos + epNum)))
        {
            ...
            if (m_InEndpoints[epNum].DIEPINT & USB_OTG_DIEPINT_XFRC)
            {
                ...
                if (epNum == 0 && m_Enpoint0InPending != 0) {
                    ...
                    IRQTransferComplete(epNum | USB_ADDRESS_DIR_IN, xfer.TotalLength, USB_TransferResult::Success);
                }
            }
            // TX FIFO empty.
            if ((m_InEndpoints[epNum].DIEPINT & USB_OTG_DIEPINT_TXFE) && (m_Device->DIEPEMPMSK & (1 << epNum)))
            {
                ...
                for (uint32_t i = 0; i < remainingPackets; ++i)
                {
                    ...
                    WriteToFIFO(epNum, xfer.Buffer, packetSize);
