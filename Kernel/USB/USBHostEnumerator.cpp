...
void USBHostEnumerator::HandleConfigurationHeaderResult(bool result, uint8_t deviceAddr)
{
    ...
    {
        kernel_log(LogCategoryUSBHost, KLogSeverity::ERROR, "USBH: Get device descriptor request failed.\n");
...
void USBHostEnumerator::HandleConfigurationFullResult(bool result, uint8_t deviceAddr)
{
    if (result)
    {
        ...
        if (device != nullptr)
        {
            kernel_log(LogCategoryUSBHost, KLogSeverity::INFO_LOW_VOL, "USBH: PID: %04xh\n", int(device->m_DeviceDesc.idProduct));
            kernel_log(LogCategoryUSBHost, KLogSeverity::INFO_LOW_VOL, "USBH: VID: %04xh\n", int(device->m_DeviceDesc.idVendor));

            m_HostHandler->GetControlHandler().ReqSetAddress(device->m_Address, bind_method(this, &USBHostEnumerator::HandleSetAddressResult));
        }
        else
        {
            kernel_log(LogCategoryUSBHost, KLogSeverity::ERROR, "USBH: Failed to allocate new device node.\n");
            SendResult(result, deviceAddr);
        }
    }
    else
    {
        kernel_log(LogCategoryUSBHost, KLogSeverity::ERROR, "USBH: Get full device descriptor request failed.\n");
        SendResult(result, deviceAddr);
    }
}
...
void USBHostEnumerator::HandleSetAddressResult(bool result, uint8_t deviceAddr)
{
    if (result)
    {
        ...
        if (device != nullptr)
        {
            kernel_log(LogCategoryUSBHost, KLogSeverity::INFO_LOW_VOL, "USBH: Address %d assigned.\n", device->m_Address);
            ...
            control.ReqGetDescriptor(deviceAddr, USB_RequestRecipient::DEVICE, USB_RequestType::STANDARD, USB_DescriptorType::CONFIGURATION, 0, 0, buffer, sizeof(USB_DescConfiguration),
                [this, buffer, device, &control](bool result, uint8_t deviceAddr)
                {
                    if (result)
                    {
                        ...
                        control.ReqGetDescriptor(deviceAddr, USB_RequestRecipient::DEVICE, USB_RequestType::STANDARD, USB_DescriptorType::CONFIGURATION, 0, 0, buffer, configHeader->wTotalLength,
                            [this, buffer, device](bool result, uint8_t deviceAddr)
                            {
                                ...
                                {
                                    kernel_log(LogCategoryUSBHost, KLogSeverity::ERROR, "USBH: Failed to get full configuration descriptor.\n");
                                    SendResult(result, deviceAddr);
                                }
                    ...
                    {
                        kernel_log(LogCategoryUSBHost, KLogSeverity::ERROR, "USBH: Failed to get configuration descriptor header.\n");
                        SendResult(result, deviceAddr);
                    }
                }
            );
    ...
    {
        kernel_log(LogCategoryUSBHost, KLogSeverity::ERROR, "USBH: Failed to set device address.\n");
        SendResult(result, deviceAddr);
    }
}
